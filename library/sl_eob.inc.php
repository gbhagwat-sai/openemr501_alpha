...
function slInvoiceNumber(&$out)
{
	...
	// Code change by Mahesh Kunta. removed PID from ERA posting for DHC client
    ...
    if ($pid && $encounter && $_SESSION["site_id"] === "dhc" ) {
        $invnumber = "$encounter";
    } else if($pid && $encounter){
...
//PAYEHR-288 change start here
	...
	{		
		if($insuCount == 2 && $balance != $charge && $balance > 0.00 && $pri > 0.00) //Billed to secondary
		{
			...
			sqlStatement("update form_encounter set claim_status_id = '7', last_level_billed = 2, last_level_closed = 1, modified_date = '$todays_date', modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
	...
	else
	{
		if($insuCount == 1 && ($pri != 0 || $pricoins != 0 || $writeoff != 0 || $prideduct != 0)){
			if($insuCount == 1 && $balance == 0.00) // claim settled
			{
				...
				sqlStatement("update form_encounter set claim_status_id = '12', last_level_billed = 1, last_level_closed = 1, modified_date = '$todays_date', modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}
			else if($insuCount == 1 && $balance < 0.00) //Excess Payment
			{

				sqlStatement("update form_encounter set claim_status_id = '15', last_level_billed = 1, last_level_closed = 1, modified_date = '$todays_date', modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}
			else if($insuCount == 1 && $balance != $charge && $pri == 0.00 && $prideduct > 0.00) //Ready to send patient
			{
				
				sqlStatement("update form_encounter set claim_status_id = '10', last_level_billed = 1, last_level_closed = 1, modified_date = '$todays_date', modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}
			...
			if($insuCount == 2 && $balance == 0.00) // claim settled
			{
				...
				sqlStatement("update form_encounter set claim_status_id = '12', last_level_billed = 2, last_level_closed = 2, modified_date = '$todays_date', modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}
			else if($insuCount == 2 && $balance < 0.00) // Excess Payment
			{
				
				sqlStatement("update form_encounter set claim_status_id = '15', last_level_billed = 2, last_level_closed = 2, modified_date = '$todays_date',modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}
			else if($insuCount == 2 && $balance > 0.00 && $pri > 0.00 && $sec == 0.00) //Ready to send secondary
			{
				sqlStatement("update form_encounter set claim_status_id = '6',modified_date='$todays_date', last_level_billed = 1, last_level_closed = 1, modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}
			else if($insuCount == 2 && $balance != $charge && $balance > 0.00 && $pri == 0.00 && $prideduct > 0.00) //Ready to send secondary
			{
				
				sqlStatement("update form_encounter set claim_status_id = '6', last_level_billed = 1, last_level_closed = 1, modified_date = '$todays_date', modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}
			else if($insuCount == 2 && $balance != $charge && $balance > 0.00 && $pri > 0.00 && $sec == 0.00 && $secdeduct > 0.00 ) //Ready to send patient
			{
				
				sqlStatement("update form_encounter set claim_status_id = '10', last_level_billed = 2, last_level_closed = 2, modified_date = '$todays_date', modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}
		}
			...
			if($insuCount == 3 && $balance == 0.00) // claim settled
			{
				...
				sqlStatement("update form_encounter set claim_status_id = '12', last_level_billed = 3, last_level_closed = 3, modified_date = '$todays_date', modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}	
			else if($insuCount == 3 && $balance != $charge && $balance < 0.00) // Excess Payment
			{
				
				sqlStatement("update form_encounter set claim_status_id = '15', last_level_billed = 3, last_level_closed = 3, modified_date = '$todays_date', modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}
			else if($insuCount == 3 && $balance != $charge && $pri > 0.00 && $sec == 0.00 && $secdeduct > 0.00) //Ready to send tertiary
			{
				
				sqlStatement("update form_encounter set claim_status_id = '8', last_level_billed = 2, last_level_closed = 2, modified_date = '$todays_date', modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}
			...
			{
				sqlStatement("update form_encounter set claim_status_id = '10', last_level_billed = 3, last_level_closed = 3, modified_date = '$todays_date', modified_by = $modified_by where encounter = '$encounter' and pid = '$patient_id'");
			}
