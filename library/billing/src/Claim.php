...
// Handles date time stamp formats as well
  ...
  //
    public function __construct($pid, $encounter_id)
    {
        ...
        //Updatd the query by Gangeya for PAYEHR-532
      ...
      $provider_id = $this->procs[0]['provider_id'];
        *****************************************************************/
        ...
        //If encounter have servicing provider it replaces the rendering provider.
        //PAYEHR-532 code added by Gangeya
        if($this->encounter['servicing_provider_id'] != 0){
            $provider_id = $this->encounter['servicing_provider_id'];
        }
        else{
            $provider_id = $this->encounter['provider_id'];
        }
        ...
        //Added by Gangeya for PAYEHR-532 to populate referring provider from encounter table
  ...
  //
    public function payerAdjustments($ins, $code = 'Claim')
    {
        ...
        if (!empty($this->invoice[$code])) {
            ...
            $charge =  $this->invoice[$code]['chg'];
            $ptresp = $this->invoice[$code]['chg'] + $this->invoice[$code]['adj'];
            $adjustment = $this->invoice[$code]['adj'];

            foreach ($this->invoice[$code]['dtl'] as $key => $value) {
                // plv (from ar_activity.payer_type) exists to
                // indicate the payer level.
                $rsn = $value['rsn'];
                //PAYEHR 532 Code added by Gangeya to fetch Co-Insurance amount
                $str = '/Co-ins/i';
                if(preg_match($str, $rsn)){
                    $coinsurance = substr($rsn, strpos($rsn, ": ") + 2); 
                }
            ...
            foreach ($this->invoice[$code]['dtl'] as $key => $value) {
                ...
                if ($tmp && $value['pmt'] == 0) { // not original charge and not a payment
                    $rsn = $value['rsn'];
                    //PAYEHR 532 Code added by Gangeya to fetch Write-Off amt
                    if($value['chg'] == 0.00 && $value['wr_off'] != 0.00){
                        $chg = $value['wr_off']; // Wrote OFF 
                        $wr_off = $value['wr_off'];
                    }else{
                        $chg = 0 - $value['chg']; // adjustments are negative charges
                    }
                    //PAYEHR 532 Code added by Gangeya to fetch Deductible amount
                    $str = '/Deductible/i';
                    if(preg_match($str, $rsn)){
                        $deductible = substr($rsn, strpos($rsn, "$") + 1); 
                    }

                    
                    //PAYEHR 627 Code added by Gangeya to add other adjustment
                    $str = '/Ins adjust/i';
                    if(preg_match($str, $rsn)){
                        $gcode = 'OA';
                        $rcode = '23';
            ...
            //if ($thispaidanything) {
            // echo 'Charge: '.$charge.' Deductible: '.$deductible.' Coins: '.$coinsurance.' Write Off: '.$wr_off.' Paid Amt: '.$thispaidanything.' Adjustment: '.$adjustment.'<<';                            
            // echo $calBal;
            // echo '<br>';

            $calBal = $charge - ($thispaidanything + $wr_off);
            //echo '<br \>'.'Calucalated Balence: '.$calBal;
            if ($thispaidanything) {
                $coinsurance;
            } 
            else {
...
//adding in public functions for isa 01 - isa 04
    ...
    //Updatd the function by Gangeya for PAYEHR-532
    public function x12gsisa02()
    {
        //return $this->x12_partner['x12_isa04'];
        $tmp = $this->x12_partner['x12_isa02'];
        while (strlen($tmp) < 10) {
                $tmp .= " ";
        }
    ...
    {
        //return $this->x12_partner['x12_isa04'];
        $tmp = $this->x12_partner['x12_isa04'];
        while (strlen($tmp) < 10) {
                $tmp .= " ";
        }
...
/////////
    ...
    //PAYEHR-532 update Pay-To Address logic by Gangeya
    //Pay To Address code START

    public function payToStreet()
    {
        return $this->x12Clean(trim($this->billing_facility['payTostreet']));
    }

    public function payToCity()
    {
        return $this->x12Clean(trim($this->billing_facility['payTocity']));
    }

    public function payToState()
    {
        return $this->x12Clean(trim($this->billing_facility['payTostate']));
    }

    public function payToZip()
    {
        return $this->x12Clean(trim($this->billing_facility['payTopostal_code']));
    }

    //Pay to Address code END
  ...
  # The billing facility and the patient must both accept for this to return true.
    ...
    public function billingProviderNPI24J($prockey = -1)
    {
        $tmp = ($prockey < 0 || empty($this->procs[$prockey]['provider_id'])) ?
        $this->insurance_numbers : $this->procs[$prockey]['insurance_numbers'];
        return $this->x12Clean(trim(str_replace('-', '', $tmp['practice_npi'])));
        //$this->insurance_numbers['practice_npi'];
    }

    public function box25Type($prockey = -1)
    {
        $tmp = ($prockey < 0 || empty($this->procs[$prockey]['provider_id'])) ?
        $this->insurance_numbers : $this->procs[$prockey]['insurance_numbers'];
        return $this->x12Clean(trim(str_replace('-', '', $tmp['federal_tax_id_number_type'])));
        //$this->insurance_numbers['practice_npi'];
    }

    public function box25ProviderNumber($prockey = -1)
    {
        $tmp = ($prockey < 0 || empty($this->procs[$prockey]['provider_id'])) ?
        $this->insurance_numbers : $this->procs[$prockey]['insurance_numbers'];
        return $this->x12Clean(trim(str_replace('-', '', $tmp['federal_tax_id_number'])));
        //$this->insurance_numbers['practice_npi'];
    }
