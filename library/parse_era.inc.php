...
function parse_era($filename, $cb)
{
    ...
    while (true) {
	  ...
	  //changes made for bug 8741
	     $buffer = str_replace( array("\r\n"), '', $buffer); // Sai custom code
    ...
    // If this is the ISA segment then figure out what the delimiters are.
        ...
        if ($segid == 'ISA') {
	       ...
	       // sai custom code start
						//Below condition added by sangram for bug 9104 
			if(strpos($seg[2]," ") !== false) 
			{
			$chk_no= (substr($seg[2],(strpos($seg[2]," "))));
			$seg[2]=$chk_no;
			}
		// sai custom code end		
        ...
        //
            ...
            for ($k = 2; $k < 20; $k += 3) {
				...
				// sai custom code start
				//By Gangeya for BUG ID 10287
				//added new adjustment Code to capture write-off
				if($seg[1]=="CO" && ($seg[$k]==45 || $seg[$k]==119))
				$out['svc'][$i]['adj'][$j]['w_o'] = $out['svc'][$i]['adj'][$j]['amount'];
				// sai custom code end
            }
        } // QC = Patient
