...
function gen_x12_837($pid, $encounter, &$log, $encounter_claim = false)
{
    ...
    if (($claim->billingFacilityNPI() && $claim->billingFacilityETIN()) && empty($claim->box25ProviderNumber())) {
        ++$edicount;
        $out .= "REF";
    ...
    //2010AA Box No 25 SSN number or ETIN
    if ($claim->box25Type()) {
        ++$edicount;
        $out .= "REF" .
            "*" . $claim->box25Type() .
            "*" . $claim->box25ProviderNumber() .
            "~\n";
    } else {
        $log .= "*** No billing facility NPI and/or ETIN.\n";
    }
    
    //commented by Gangeya.
    ...
    // above is for historical use of encounter onset date, now in misc_billing_options
    //commented the IF clause to allow to show hospitalization date PAYEHR-601
    //if (strcmp($claim->facilityPOS(), '21') == 0 && $claim->hospitalizedFromDateValid()) {
    if ($claim->hospitalizedFromDateValid()) {
        ++$edicount;
    ...
    if ($claim->hospitalizedToDateValid()) {
        ++$edicount;
    ...
    if ($claim->providerNPIValid() && ($claim->billingProviderNPI24J() !== $claim->providerNPI())) {
        ++$edicount;
        ...
        "*" .
        "*";
        if ($claim->billingProviderNPI24J()) {
            $out .=
            "*" . "XX" .
            "*" . $claim->billingProviderNPI24J();
        } else {
    ...
    //
        if ($claim->providerNPI() != $claim->billingProviderNPI24J($prockey)) {
            ++$edicount;
