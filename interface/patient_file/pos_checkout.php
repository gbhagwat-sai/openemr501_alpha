...
if ($_POST['form_save']) {
  ...
  //
    ...
    $lines = $_POST['line'];
  $Check_date= date("Y-m-d",strtotime($_POST['form_check_date'])) ;// sai custom code
    ...
    for ($lino = 0; $lines[$lino]['code_type']; ++$lino) {
        ...
        if ($code_type == 'PROD') {
            ...
            // script's input form.
      addBillingForCheckout($form_encounter, 'TAX', 'TAX', 'Taxes', $form_pid, 0, 0,
        '', 1, $amount, '', '', 1,$Check_date); // Sai custom code
  ...
  $Check_date= date("Y-m-d",strtotime($_POST['form_check_date'])) ; // sai custom code
	...
	// sai custom code start
	      ...
	      addBillingForCheckout($form_encounter, 'COPAY', $amount, $paydesc, $form_pid,
	        1, 0, '', 1, 0 - $amount, '', '', 1,$Check_date); // Sai custom code
	 // sai custom code end
...
<?php } ?>
  ...
  f.form_amount1.value = total.toFixed(<?php echo $currdecimals ?>); // sai custom code 
  return true;
 }
...
if (confirm("Discount Amount :"+  document.getElementById('form_discount').value + "\n\n Payment Method :"+ document.getElementById('form_method').value + "\n\n Check/Reference Number :"+  document.getElementById('form_source').value + "\n\n Amount :"+  document.getElementById('form_amount').value + "\n\n Posting Date: "+  document.getElementById('form_date').value + "\n")) {
return true;
}
...
document.getElementById('form_check_date').disabled=true;
document.getElementById('img_check_date').style.visibility = 'hidden';
document.getElementById('form_check_date').style.visibility = 'hidden';
document.getElementById('Check_Date_Lbl_Div').style.visibility = 'hidden';

}
function PaymentMetodChanged(obj)
{
//alert(obj.value);
if(obj.value=='Check')
{
document.getElementById('form_check_date').value='';
document.getElementById('form_check_date').disabled=false;
document.getElementById('img_check_date').style.visibility = 'visible'; 
document.getElementById('form_check_date').style.visibility = 'visible';
document.getElementById('Check_Date_Lbl_Div').style.visibility = 'visible';
}
else
{
document.getElementById('form_check_date').value='';
...
<body class="body_top" onLoad="LoadingForm();">  <!-- sai custom code start -->

<form method='post' action='pos_checkout.php' onSubmit="return executeOnSubmit();"><!-- sai custom code start -->
...
<table border='0' cellspacing='4'>
 ...
 <tr>
  ...
  <td>
   <input type='text' name='form_discount' id="form_discount" size='6' maxlength='8' value=''
    style='text-align:right' onkeyup='computeTotals()'><!-- sai custom code start -->
  </td>
 </tr>
 ...
 <tr>
  ...
  <td>
   <select name='form_method' id="form_method" onChange="PaymentMetodChanged(this);"><!-- sai custom code  -->
    <?php
  ...
  <!-- sai custom code start -->
  <td>
   <div id='Check_Date_Lbl_Div'><?php xl('Check date','e'); ?>:</div>
  </td>
  <td>
   <input type='text' size='10' name='form_check_date' id='form_check_date'
    value='<?php echo date('m/d/Y',strtotime($inv_date)) ?>'
    
    title='yyyy-mm-dd date of service'
    onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc,'<?php echo date("m/d/Y"); ?>')' />
   <img src='../pic/show_calendar.gif' align='absbottom' width='24' height='22'
    id='img_check_date' border='0' alt='[?]' style='cursor:pointer'
 ...
 <tr>
  <td>
<!--  rename 'Amount paid' to 'Check amount'-->
   <?php xl('Check/Cash/Card amount','e'); ?>: 
  </td><!-- sai custom code start -->
  <td>
   <input type='text' name='form_amount' id="form_amount" size='10' value='0.00'>
  </td><!-- sai custom code  -->
 </tr>
 ...
 <tr>
  ...
  <td>
   <input type='text' size='10' class='datepicker' name='form_date' id='form_date'
    value='<?php echo date('m/d/Y',strtotime($inv_date)) ?>'
    title='yyyy-mm-dd date of service' />
    onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc,'<?php echo date("m/d/Y"); ?>')' /><!-- sai custom code  -->
   <img src='../pic/show_calendar.gif' align='absbottom' width='24' height='22'
...
?>
 ...
 <tr>
  <td colspan='2' align='center'>
   &nbsp;<br>
   <input type='submit' name='form_save' value='<?php xl('Post the payment','e'); ?>' /> &nbsp;<!-- sai custom code  -->
