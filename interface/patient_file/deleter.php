...
require_once($GLOBALS['srcdir'].'/sl_eob.inc.php');
 ...
 $info_msg = "";
  $userID = $_SESSION['authUserID']; // Sai custom code
  $todays_date = date('Y-m-d H:i:s'); // Sai custom code
...
function form_delete($formdir, $formid, $patient_id, $encounter_id)
{
    ...
    if (substr($formdir, 0, 3) == 'LBF') {
        ...
        //row_modify("form_$formdir" , "isActive = 0, modified_date=NOW(), modified_by=$userID "  , "forms_id = '" . add_escape_custom($formid) . "'");
    } else {
        //row_delete("form_$formdir", "id = '" . add_escape_custom($formid) . "'");        
        row_modify("form_$formdir" , "isDeleted = 0, modified_date=NOW() "  , "id = '" . add_escape_custom($formid) . "'");
    }
}
...
if ($_POST['form_submit']) {
    if ($patient) {
        if (!acl_check('admin', 'superbill') || !$GLOBALS['allow_pat_delete']) {
             die("Not authorized!");
        }
           // Sai custom code start
            // code added for log deleted encounter details into table deletedEncounters
          $row = sqlQuery("SELECT billEHRPatientID FROM patient_data WHERE pid= '$patient'");
          $billEHRPatientID = $row['billEHRPatientID'];
         
          
          $query1 = "INSERT INTO deletedpatient(`pmsPatientID`, `billEHRPatientID`, `deletedDate`, `deletedBy`) VALUES ('$patient', '$billEHRPatientID', '$todays_date', '$userID')";

          sqlStatement($query1); 
  
          /************************************************************************/
        // code added for log deleted encounter details into table deletedEncounters
          $res = sqlStatement("SELECT * FROM form_encounter WHERE pid = '$patient'");
           
          while ($row = sqlFetchArray($res)) {
              $encounter = $row['encounter'];

              $row1 = sqlQuery("SELECT billEHREncounterID FROM form_encounter WHERE encounter = '$encounter'");
              $billEHREncounterID = $row1['billEHREncounterID'];

          
              $query2 = "INSERT INTO deletedencounters(`pmsEncounterID`, `billEHREncounterID`, `deletedDate`, `deletedBy`) VALUES ('$encounter', '$billEHREncounterID', '$todays_date', '$userID')";
              sqlStatement($query2);  

           }
           /******************************************************************/
            // Sai custom code end
            row_modify("billing"       , "activity = 0,modified_date=NOW(),modified_by=$userID", "pid = '$patient'");// Sai custom code
            row_modify("billing_draft", "activity = 0", "pid = '" . add_escape_custom($patient) . "'");// Sai custom code
    	    ...
    	    //row_delete("pqrs_reported"    , "pid = '" . add_escape_custom($patient) . "'"); // Sai custom code
            ...
            //echo '<br />';
            //echo '<pre>';
            //echo "SELECT * FROM forms WHERE pid = $patient";
            //print_r($patient);
            while ($row = sqlFetchArray($res)) {
                //echo $row['encounter'];echo '<br/>';
                form_delete($row['formdir'], $row['form_id'], $row['pid'], $row['encounter']);
            }
            //die;
            row_modify("forms" , "active = 0"  , "pid = '$patient'");
            row_modify("form_encounter_draft" , "active = 0"  , "pid = '$patient'");
    	    //row_delete("forms", "pid = '" . add_escape_custom($patient) . "'");
    	    //row_delete("form_encounter_draft", "pid = '" . add_escape_custom($patient) . "'"); // Sai custom code
            ...
            //}
            row_modify("patient_data" , "active = 0,modified_date=NOW(),modified_by=$userID"  , "pid = '$patient'");
        ...
        // Sai custom code start
           // code added for log deleted encounter details into table deletedEncounters
          $row = sqlQuery("SELECT billEHREncounterID FROM form_encounter WHERE encounter = '$encounterid'");
          $billEHREncounterID = $row['billEHREncounterID'];
         
          
          $query1 = "INSERT INTO deletedencounters(`pmsEncounterID`, `billEHREncounterID`, `deletedDate`, `deletedBy`) VALUES ('$encounterid', '$billEHREncounterID', '$todays_date', '$userID')";

          sqlStatement($query1); 
            echo $userID;die;
            row_modify("billing", "activity = 0,modified_date=NOW(),modified_by=$userID", "encounter = '" . add_escape_custom($encounterid) . "'");
            row_modify("billing_draft", "activity = 0", "encounter = '$encounterid'");
            //delete_drug_sales(0, $encounterid);
            //row_delete("pqrs_reported", "encounter = '" . add_escape_custom($encounterid) . "'");//added for PQRS delete
            ...
            row_modify("forms" , "active = 0"  , "encounter = '" . add_escape_custom($encounterid) . "'");
            row_modify("form_encounter_draft" , "active = 0"  , "encounter = '" . add_escape_custom($encounterid) . "'");
        	//row_delete("forms", "encounter = '" . add_escape_custom($encounterid) . "'");
        	//row_delete("form_encounter_draft", "encounter = '" . add_escape_custom($encounterid) . "'");
        	 // Sai custom end
    } else if ($formid) {
        if (!acl_check('admin', 'superbill')) {
            die("Not authorized!");
	 ...
	 // Sai custom code start
		row_modify("billing_draft", "activity = 0",
        "pid = '" . add_escape_custom($patient_id) . "' AND " .
        "encounter = '" . add_escape_custom($encounter_id) . "' AND " .
        "code_type = 'COPAY' AND " .
        "activity = 1");
	...
	sqlStatement("UPDATE form_encounter_draft SET last_level_billed = 0, " .
        "last_level_closed = 0, stmt_count = 0, last_stmt_date = NULL " .
