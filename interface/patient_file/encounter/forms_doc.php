...
if (is_numeric($pid)) {
    ...
    echo htmlspecialchars( xl('Documentation for','',' ',' ') . $result['fname'] . " " . $result['lname'] );
    if ($result['squad'] && ! acl_check('squads', $result['squad'])) {
        $auth_notes_a = $auth_notes = $auth_relaxed = 0;
    }
    // Check  no access to the encounter's sensitivity level.
...
<input  name="Assign_Documentation" type="submit" value="Back">
</form>

<?php

  if ($result = getFormByEncounter($pid, "000000", "id, date, form_id, form_name, formdir, user, deleted")) {
...
// Added By Gangeya to assign Documentation to selected Encounter.
		...
		if (acl_check('admin', 'super') ) {
            if ( $formdir != 'newpatient') {
                ...
                echo "<a target='".
                    ($GLOBALS['concurrent_layout'] ? "_parent" : "Main") .
                    "' href='$rootdir/patient_file/encounter/assign_form.php?" .
                    "formname=" . $formdir .
                    "&id=" . $iter['id'] .
                    "&encounter=". $encounter.
                    "&pid=".$pid.
                    "' class='css_button' title='" . xl('Assign Encounter') . "' onclick='top.restoreSession()'><span>" . xl('Assign') . "</span></a>";
            } else {
                ?><a href='javascript:;' class='css_button' style='color:gray'><span><?php xl('Assign','e'); ?></span></a><?php
            }
        }
