...
// Sai custom code start
 if($_REQUEST['mode'] && $_REQUEST['mode']=="edit"){
  	$issue = $_GET['issue'];
 }
// Sai custom code end
...
if ($_POST['form_save']) {
  ...
  // Sai custom code start
$query1=sqlStatement("SELECT * FROM lists where title='".$_POST['form_title']."' and diagnosis='".$_POST['form_diagnosis']."' and pid='".$thispid."'");
...
// Sai custom code end
  ...
  } else if($num_rows<1) { // Sai custom code 
...
?>
 ...
 function add_edit_diagnosis() {
  dlgopen('../encounter/superbill_custom_full.php', '_blank', 700, 600);
  return false;
 }
// Sai custom code end
