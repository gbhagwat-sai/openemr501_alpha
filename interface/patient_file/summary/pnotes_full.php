...
$pres = getPatientData($patient_id, "lname, fname,pubpid");
$patientname = $pres['lname'] . ", " . $pres['fname'];

$external_id = $pres['pubpid']; // sai custom code
//retrieve all notes
...
function show_div(name){
  if(name == 'inbox'){
    ...
    document.getElementById('outbox_div').style.display = 'none';
	 document.getElementById('statement_div').style.display = ''; // sai custom code
  }else{
    document.getElementById('inbox_div').style.display = 'none';
    document.getElementById('outbox_div').style.display = '';
	 document.getElementById('statement_div').style.display = 'none'; // sai custom code
  }
}
...
<div class='tabContainer' >
  <div id='statement_div' <?php echo $inbox_style; ?> ><!-- sai custom code -->
...
if ($result != "") {
	...
	$GLOBALS['date_display_format'] = 1; // sai custom code
...
<!--code added for statement log patient statement phase-II BuG ID 10485 start-->
<div id='inbox_div' <?php echo $inbox_style; ?> >
<!-- code added for showing statement log for BUG ID 10485  start---------->
    <h4><span style="color:#0000CC">Statement Log</span></h4>
	...
	$log_result = getStatementLog($external_id, $N, 0);
	 ...
	 if ($log_result != null) {
	 	...
	 	$log_count = 0;//number of notes so far displayed
	 	foreach ($log_result as $iter) {
    		...
    		echo  "<td  valign='top' class='text' style='font-size:12px'>";
			echo "Patient Statement Printed for <b>$".$balance1."</b>";
			echo  " on ".$date2." for Statement <a href='$pdf_path' target='_blank' title='View Statement'>(".$statement_id.")</a> by User";
			echo  " (".$iter['user'].") .</td></tr>";
...
<!--code added for statement log patient statement phase-II BuG ID 10485 end-->
<!-- sai custom code end -->
