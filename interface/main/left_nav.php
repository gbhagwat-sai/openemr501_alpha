...
'enh' => array(xl('Encounter Summary'), 0, 'forms/newpatient/encounter_summary.php')// Sai custom code
);
$disallowed = array();
  ...
  if(f.sel_frame)
   {
	  ...
	  if (fi == 1) frame = 'RTop'; else if (fi == 2) frame = 'RTop'; // Sai custom  code 
   }
  if (!f.cb_bot.checked) frame = 'RTop'; else if (!f.cb_top.checked) frame = 'RTop'; // Sai custom  code  
  top.frames[frame].location = '<?php echo "$web_root/interface/" ?>' + url;
  if (frame == 'RTop') topName = fname; else botName = fname;
  return false;
 }
...
function clearactive() {
 ...
 function setPatient(pname, pid, pubpid, frname, str_dob,note) { // sai custom code

    //console.log('pid'+pid+'name'+pname);
  clearTherapyGroup();
  ...
  setDivContent('current_patient', str);
   ...
   var data= str_dob + '<br /> ' + note.fontcolor("red").fontsize(2);
  setTitleContent('current_patient', str + data); // sai custom code
...
<?php if (acl_check_form('newpatient', '', array('write', 'addonly'))) { ?>
 ...
 str+='<option value="Unbilled Encounter"><?php echo htmlspecialchars( xl('Unbilled Encounter List'), ENT_QUOTES) ?></option>'; // sai custom code
<?php } ?>
  str+='<option value="Past Encounter List"><?php echo htmlspecialchars(xl('Past Encounter List'), ENT_QUOTES) ?></option>';
  if(EncounterIdArray && EncounterDateArray && CalendarCategoryArray){ // sai custom code
...
<table cellpadding='0' cellspacing='0' border='0' width='100%'>
 <tr>
  <td class='smalltext' nowrap>
   <input type='checkbox' name='cb_top' onclick='toggleFrame(1)' checked  disabled/> <!-- sai custom code -->
   <b><?php xl('Top', 'e') ?></b>
  </td>
  <td class='smalltext' align='right' nowrap>
   <b><?php xl('Bot', 'e') ?></b>
   <input type='checkbox' name='cb_bot' onclick='toggleFrame(2)' <?php if (empty($GLOBALS['athletic_team'])) echo ''; ?> /><!-- Sai custom code -->
  </td>
 </tr>
</table>
...
<?php } // end if gbl_nav_visit_forms ?>
  ...
  <li><a class="collapsed" id="feeimg" ><span><?php xl('Fees', 'e') ?></span></a>
    <ul>
        <?php if (acl_check('encounters', 'coding')) {
	       ...
	       genMiscLink('RTop','enh','0',xl('Encounter Summary'),'forms/newpatient/encounter_summary.php');  // sai custom code 
} ?>
  ...
  <?php genMiscLink('RTop','enh','0',xl('Printed Claims'),'billing/paperClaimReport.php'); // Sai custom code ?>
  ...
  <li><a class="collapsed" id="admimg" ><span><?php xl('Administration', 'e') ?></span></a>
    <ul>
        ...
        <?php if (acl_check('admin', 'users')) {
	       ...
	       genMiscLink('RTop','adm','0',xl('Patient Statement'),'usergroup/statement_conf.php'); // Sai custom code
      genMiscLink('RTop','ens','0',xl('Status Update'),'orders/status_update.php'); // sai custom code
} ?>
...
?>
  ...
  <!-- Coed Added for Patient statement-->
 <!-- sai custom code start -->
  <li><a class="collapsed" id="feeimg" ><span><?php xl('Patient Statement','e') ?></span></a>
    <ul>
      <?php genMiscLink('RTop','rep','0',xl('Individual Statement'),'billing/sl_eob_search_patient.php'); ?>
      <?php genMiscLink('RTop','rep','0',xl('Batch Statement'),'billing/search_batch_patient.php'); ?>
      
      <?php genMiscLink('RTop','rep','0',xl('Printed Statement'),'billing/statement_search.php'); ?>
    </ul>
  </li>
 <!-- sai custom code end -->
   <li><a class="collapsed"><span><?php xl('Saicle One Reports','e') ?></span></a>
        <ul>

        <?php genMiscLink('RTop','rep','0',xl('Day-Sheet'),'orders/daysheet_report.php'); ?>
        <?php genMiscLink('RTop','rep','0',xl('Facesheet'),'orders/facesheet_report.php'); ?>
        <?php genMiscLink('RTop','rep','0',xl('Progress Note'),'orders/pn_report.php');  ?>
