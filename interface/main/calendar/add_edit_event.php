            ...
            if ($GLOBALS['select_multi_providers'] && $row['pc_multiple']) {
                ...
                if ($_POST['recurr_affect'] == 'current') {
                    ...
                    foreach ($_POST['form_provider'] as $provider) {
						...
						"pc_modifiedDate = NOW(), " .
						"pc_modifiedBy = '" . $_SESSION['authUserID'] . "' " .
...
*********************************************************************/
 ...
 // If we are editing an existing event, then get its data.
    if ($eid) {
        ...
        $row = sqlQuery("SELECT e.*, u.fname, u.mname, u.lname, DATE_FORMAT(pc_time, '%m/%d/%Y %H:%i:%s') as pc_time " .
          "FROM openemr_postcalendar_events AS e " .
          "LEFT OUTER JOIN users AS u ON u.id = e.pc_informant " .
          "WHERE pc_eid = ?", array($eid));
        $informant = $row['fname'] . ' ' . $row['mname'] . ' ' . $row['lname'];
        $createdDate = $row['pc_time'];

		$row = sqlQuery("SELECT e.*, u.fname, u.mname, u.lname, DATE_FORMAT(pc_modifiedDate, '%m/%d/%Y %H:%i:%s') as pc_modifiedDate ".
			"FROM openemr_postcalendar_events AS e " .
			"LEFT OUTER JOIN users AS u ON u.id = e.pc_modifiedBy " .
			"WHERE pc_eid = $eid");
		$modifier = $row['fname'] . ' ' . $row['mname'] . ' ' . $row['lname'];
		$modifiedBy = $row['pc_modifiedBy'];
		$modifiedDate = $row['pc_modifiedDate'];
 ...
 // BUG ID 10635 : for reflecting only rendering providers on event scheduler
    $ures = sqlStatement("SELECT id, username, fname, lname FROM users WHERE " .
    "authorized != 0 AND active = 1 AND newcrop_user_role = 'erxrenderingProvider' AND calendar = 1 ORDER BY lname, fname");
          ...
          echo "<option value={$facrow['id']} $selected>{$facrow['name']}</option>";
            *************************************************************/
            if ($_SESSION['authorizedUser'] || in_array($facrow, $facils)) {
                //$selected = ( $facrow['id'] == $e2f ) ? 'selected="selected"' : '' ;
                if($_SESSION[site_id] = 'mtc'){
                    $selected = ( $facrow['id'] == 104 ) ? 'selected="selected"' : '' ;
                }else{
                    $selected = ( $facrow['id'] == $e2f ) ? 'selected="selected"' : '' ;
                }
...
<?php if ($informant) echo "<p class='text'>" . xl('Created By') . "<b> $informant </b>". xl(' On ') ."<b> $createdDate</b></p>\n"; ?>
<?php if($modifiedBy != 0) echo "<p class='text'>" . xl('Last Updated By') . "<b> $modifier </b>". xl(' On ') ." <b>$modifiedDate </b></p>\n"; ?>
<?php if ($informant) {
...
$(function (){
    ...
    $("#all_events").click(function() { alert ('1'); $("#recurr_affect").val("all"); EnableForm(); SubmitForm(); });
    $("#future_events").click(function() { alert ('2'); $("#recurr_affect").val("future"); EnableForm(); SubmitForm(); });
    $("#current_event").click(function() { alert ('3'); $("#recurr_affect").val("current"); EnableForm(); SubmitForm(); });
    $("#recurr_cancel").click(function() { alert ('4'); $("#recurr_affect").val(""); EnableForm(); HideRecurrPopup(); });
...
function validateform(event,valu){
    ...
    if ($('#form_repeat').is(':checked') || $('#days_every_week').is(':checked')){
        ...
        if(collectvalidation != undefined){
            alert('here');
...
function SubmitForm() {
    ...
    alert('1');
    if (f.form_action.value != 'delete') {alert('1.1');
