<?php /* Smarty version 2.6.31, created on 2021-02-02 09:50:42
         compiled from default/views/week/ajax_template.html */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'config_load', 'default/views/week/ajax_template.html', 10, false),array('modifier', 'date_format', 'default/views/week/ajax_template.html', 380, false),array('modifier', 'string_format', 'default/views/week/ajax_template.html', 381, false),)), $this); ?>
...
<?php 
echo "   <a href='#' title='" . xl("New Appointment") . "' onclick='newEvt(1, 9, 00, $Date, 0, 0)' class='css_button css_button_icon'>
...
<?php 
echo "   <a id='prevweek' href='$PREV_WEEK_URL' onclick='top.restoreSession()' title='" . xl("Previous Week") . "'>
            <i class='fa fa-chevron-circle-left chevron_color' aria-hidden='true'></i></a>\n";
$atmp = array_keys($A_EVENTS);
echo date('d', strtotime($atmp[0])) . " " . xl(date('M', strtotime($atmp[0]))) . " " . date('Y', strtotime($atmp[0]));
echo " - ";
echo date('d', strtotime($atmp[count($atmp)-1])) . " " . xl(date('M', strtotime($atmp[count($atmp)-1]))) . " " . date('Y', strtotime($atmp[count($atmp)-1]));
echo "   <a id='nextweek' href='$NEXT_WEEK_URL' onclick='top.restoreSession()' title='" . xl("Next Week") . "'>
            <i class= 'fa fa-chevron-circle-right chevron_color' aria-hidden='true'></i></a>\n";
 ?>
</div>
...
echo "  <a href='#' id='printview' title='" . xl("View Printable Version") . "' class='css_button css_button_icon'>
            <i class='fa fa-print' aria-hidden='true'></i></a>\n";
echo "   <a href='#' title='" . xl("Refresh") . "' onclick='javascript:refreshme()' class='css_button css_button_icon'>
...
<!-- Commented by Gangeya to hide Facility List
<?php 
...
foreach ($providers as $provider) {
    ...
    foreach ($A_EVENTS as $date => $events) {
        ...
        echo "<a href='$gotoURL'>";
        echo xl(date("D", strtotime($date))) . " " . text(oeFormatShortDate($date, false));
        echo "</a></td>";
    }
    echo " </tr>\n";
...
//         echo "<a href='javascript:newEvt($startampm,$starttimeh,$starttimem,$Date,$providerid,$in_cat_id)' title='New Appointment' alt='New Appointment'>";
    ...
    foreach ($A_EVENTS as $date => $events) {
        ...
        echo "<td class='schedule " . $classForWeekend . "' title='".date("l, d M Y",$headDate )."' date='".date("Ymd",$headDate )."' provider='$providerid'>";
