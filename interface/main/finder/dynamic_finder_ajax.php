...
// Added by Gangeya : BUG ID 8345 : Search by Comma seperated lname and fname 
	$lname = trim($_GET["sSearch_$i"]);
	$searchString = $_GET["sSearch_$i"];
    $fname = '';
	...
	{
      $where .= " `" . add_escape_custom($colname) . "` LIKE '$searchString%'";
    }
  }
}
if($where)
$where .= " and pstatus!='NO'";
else
$where .= "where pstatus!='NO' ";
...
//
$row = sqlQuery("SELECT COUNT(id) AS count FROM patient_data where pstatus!='NO'"); // Sai custom code
$iTotal = $row['count'];
