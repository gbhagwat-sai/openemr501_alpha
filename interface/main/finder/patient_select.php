...
#searchResultsHeader {
    width: 100%;
    background-color: #86d4f5; <!-- Sai custom code -->
}
...
if ($popup) {
  ...
  // Construct WHERE clause and save search parameters as form fields.
    ...
    while ($frow = sqlFetchArray($fres)) {
	    ...
	    // sai custom code start
	  else if($field_id == 'DOB'){
	  // By Gangeya BUG : 8476
	  
	  $dateA = date("Y-m-d", strtotime($value));
	  ...
	  $where .= " AND $field_id LIKE ?";
        array_push($sqlBindArray,$newDate."%");
	  }
	   else if($field_id == 'pstatus'){
	  	$value = trim($_REQUEST[$field_id]);
  ...
  // sai custom code start
 ?>
 <div style="width:auto; text-align:center;font-size: 1.0em; font-weight:bold;padding:20px 0 0 0;">
 	<label>Search Patient By Status :</label>
     <input type="checkbox" name="active_patient" value="YES" <?php if($_POST['active_patient'] == 'YES'){ echo "checked"; }?>> Active patient
     <input type="checkbox" name="inactive_patient" value="NO" <?php if($_POST['inactive_patient'] == 'NO'){ echo "checked"; }?>> Inactive patient
     <input type="submit" name="patient_status" value="Search">
 </div>
 <?php
 	//added changes for issue no 8094 (search by active/inactive user)
	if(isset($_POST)){
		$status_where = "";
		if(isset($_POST['active_patient'])){
			$status_where .= "pstatus = 'YES' AND ";
		}
		if(isset($_POST['inactive_patient'])){
			$status_where .= " pstatus = 'NO' AND ";
		}
		if($_POST['active_patient'] == 'YES' && $_POST['inactive_patient'] == 'NO'){
			$status_where = "";
		}
	}
  if ($findBy == "Last")
      $result = getPatientLnames("$patient", $given, $orderby, $sqllimit, $fstart, $status_where);
  else if ($findBy == "ID")
      $result = getPatientId("$patient", $given, "id ASC, ".$orderby, $sqllimit, $fstart,$status_where);
  else if ($findBy == "DOB")
      $result = getPatientDOB("$patient", $given, "DOB ASC, ".$orderby, $sqllimit, $fstart, $status_where);
  else if ($findBy == "SSN")
      $result = getPatientSSN("$patient", $given, "ss ASC, ".$orderby, $sqllimit, $fstart, $status_where);
  elseif ($findBy == "Phone")                  //(CHEMED) Search by phone number
      $result = getPatientPhone("$patient", $given, $orderby, $sqllimit, $fstart, $status_where);
  else if ($findBy == "Any")
      $result = getByPatientDemographics("$patient", $given, $orderby, $sqllimit, $fstart, $status_where);
  else if ($findBy == "Filter") {
    $result = getByPatientDemographicsFilter($searchFields, "$patient",
      $given, $orderby, $sqllimit, $fstart, $search_service_code, $status_where);
    }
}
  // sai custom code end
?>
...
<th class="srName"><?php echo htmlspecialchars( xl('Address'), ENT_NOQUOTES);?></th>  <!-- sai custom code -->
...
if ($result) {
    foreach ($result as $iter) {
        ...
        echo  "<td class='srName'>" . htmlspecialchars($iter['lname'] . ", " . $iter['fname']) . "</td><td class='srName'>".$iter['street'].",".$iter['postal_code'].",".$iter['city'].",".$iter['state']."</td>\n";  // sai custom code end
