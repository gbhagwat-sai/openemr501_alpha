...
?>
    ...
    <td class="detail"><?php echo $row['DXPointer' ]; ?></td>
...
<tr>
 ...
 <tr>
    ...
    <td><select name="device_type" >
            ...
            <option value="PacEHR" <?php if(isset($_POST['device_type']) && $_POST['device_type'] == "PacEHR" ){ echo "selected"; }?>>PacEHR</option>
         </select>
    </td>
...
<table border='0' cellpadding='1' cellspacing='2' width='98%'>
    <tr bgcolor="#6CAEFF" style="font-weight:bold">
        ...
        <td class="dehead"><?php xl('IP_DP','e'  ) ?></td>
        ...
        <td class="dehead"><?php xl('DXPointer','e'  ) ?></td>
...
if ($_POST['form_refresh'] || $_POST['form_csvexport']) {
	...
	if($form_claim_status == "Saved"){
		...
		group_concat(IF(b.code_type = 'ICD9' || b.code_type = 'ICD10'  ,b.code,NULL) order by b.id SEPARATOR ' , ') AS DX,
		GROUP_CONCAT(IF(b.code_type = 'CPT4', b.justify, NULL) SEPARATOR ', ') AS DXPointer,
		pl.pos_name as POS,
		SUM(b.units * b.fee) AS TotalCharge,
		cs.status as EncounterStatus,
		IF(fe.hospice = 1, 'Yes', 'No') as isHospice,
		u.createdBy as CreatedBy, 
		b.process_file as LastProcessedFile, 
		Date(b.bill_date) as TransmissionDate, 
		fe.device_type as device_type, DATE(fe.verifiedDate) as VerifiedDate, pd.Alert_note as AlertNote, fe.billEHRNotes, pd.IP_DP AS IP_DP
		...
		LEFT JOIN pos_list pl on fe.pos_id=pl.id
		join claim_status cs on  fe.claim_status_id =cs.id 		
		JOIN facility f on f.id = fe.facility_id
		LEFT JOIN facility fb on fb.id = fe.billing_facility
		where b.activity = 1 ";
	}
	else{
		$query ="select date(fe.created_date) as CreatedDate,
			...
			fe.device_type as device_type, fe.final_status as claim_status
			...
			JOIN facility f on f.id = fe.facility_id
			LEFT JOIN facility fb on fb.id = fe.billing_facility	
			where b.activity = 1 AND final_status !='save'";
}

	if($_POST['Mode'] == "DOE")
	{
		$query .= " AND fe.created_date >= '$from_date 00:00:00' AND fe.created_date <= '$to_date 23:59:59' "; 
	}
	if($_POST['Mode'] == "DOS")
	{
	...
	if($_POST['Mode'] == "DOV")
	{
		$query .= " AND fe.verifiedDate >= '$from_date 00:00:00' AND fe.verifiedDate <= '$to_date 23:59:59' ";
	}
	if ($form_batch_id) 
	{
		$query .= "AND fe.batch_id = '$batch_id' ";
	}
	if ($form_provider) 
	{
	...
	if ($form_status) 
	{
		$query .= "AND fe.claim_status_id = '$form_status' ";
	}
	/***********code added for consoludated report BUG 10521 & 10569****************/
	if ($form_device_type) 
	{
		if($form_device_type =="iPhone")
			$query .= "AND (fe.device_type like 'iP%' OR fe.device_type like 'iO%') ";
		elseif($form_device_type =="OpenEMR")
			$query .= "AND fe.device_type='OE' ";
		elseif($form_device_type =="Android")
			$query .= " AND ((fe.device_type!='OE' AND fe.device_type NOT like 'iO%' ) OR (fe.device_type!='OE' AND fe.device_type NOT like 'iO%' )) ";
		elseif($form_device_type =="PacEHR")
			$query .= "AND fe.device_type='PacEHR' ";
	}
	/******************************************************************************/
	...
	if ($form_insurance) 
	{
		$query .= "AND (select ic.id from insurance_data id join insurance_companies ic on ic.id = id.provider where id.type = 'primary' and id.pid = fe.pid and id.id in (select MAX(id) from insurance_data where type = 'primary' and pid = fe.pid) limit 1) = '$form_insurance' ";
	}
	if ($form_user) 
	{
		$query .= "AND (select ID from users where id = b.user) = '$form_user' ";
	}
	
	//added By Gangeya : BUG ID : 10775, 10894, 10898, 10902
	if($form_claim_status == "Saved")
		$query .= "group by fe.encounter order by fe.created_date desc";
	else
