...
function thisLineItem($row) {
 ...
 /* echo '"' . addslashes($row['NoteCount' ]) . '",';
    ...
    echo '"' . addslashes($row['NoteCurrentStatus' ]) . '",';
    echo '"' . addslashes(oeFormatShortDate($row['NoteStatusUpdatedDate'  ])) . '",';
    echo '"' . addslashes($row['NoteStatusUpdatedBy' ]) . '",';
    echo '"' . addslashes($row['Status' ]) . '",';
    echo '"' . addslashes($row['EncounterType' ]) . '"' . "\n";*/
  }
  else 
  {
?>
...
if ($_POST['form_csvexport']) {
  ...
  header("Content-Disposition: attachment; filename=Progress_Note_Report.csv");
...
<title><?php xl('Progress Notes Upload Report','e') ?></title>
</head>

<body leftmargin='0' topmargin='0' marginwidth='0' marginheight='0'>
<center>

<h2><?php xl('Progress Notes Upload Report','e')?></h2>

<form method='post' action='pn_report.php'>
...
if ($_POST['form_refresh'] || $_POST['form_csvexport']) {
  ...
  //$query = "CALL usp_get_ProgressNotes_Report('".$form_from_date."', '".$form_to_date."');";
