...
function checkOptionExist(value0,seperator)//It returns the position of the option, if the value is found, in the final criteria select box submitted.
 ...
 function checkOptionExist2(value0,seperator)//It returns the position of the option, if the value is found, in the final criteria select box submitted.
 {//If doesn't exist new insertion is done.If exist it is updated.
  var elSel = document.getElementById('final_this_page_criteria');
  var i;
  for (i = elSel.length - 1; i>=0; i--) {
   OptionValue=elSel.options[i].value;
   OptionValue=OptionValue.split(seperator);
   //alert(OptionValue[0]+">>>>>"+value0+">>>>>"+seperator);
    if (OptionValue[0]==value0) {
      return i;
    }
  }
...
function appendOptionRadioCriteria2(text0,value0,text1,value1,seperator,Type,query_drop_down_id)
{//If option doesn't exist new insertion is done.If exist it is updated in the select drop down.//Remove the item if the option is All.
  var elOptNew = document.createElement('option');
   //alert(seperator);
 if (Type == 'query_drop_down1')
   {
   if(query_drop_down_id =="query_drop_down_master_fe_final_status")
    var elSel1 = document.getElementById('query_drop_down_master_fe_final_status');
	if(query_drop_down_id == "query_drop_down_master_ic_id")
	 var elSel1 = document.getElementById('query_drop_down_master_ic_id');
	 if(query_drop_down_id == "query_drop_down_master_id_id")
	 var elSel1 = document.getElementById('query_drop_down_master_id_id');
	 if(query_drop_down_id == "query_drop_down_master_fe_facility")
	 var elSel1 = document.getElementById('query_drop_down_master_fe_facility');
	  if(query_drop_down_id == "query_drop_down_master_u_fname")
	 var elSel1 = document.getElementById('query_drop_down_master_u_fname');
  ...
  for (i = 0; i<=elSel1.length - 1; i++) {
   // alert("bbb");	
   
		 if (elSel1.options[i].selected){
   			 OptionValue = elSel1.options[i].text;
			 OptionValue0 = elSel1.options[i].value;			 
			 //alert(elSel1.options[i].text);						 
			 if(OptionValue1!=0 || OptionValue2!=0){
			  OptionValue1 = OptionValue1.concat(",").concat(OptionValue);
			  OptionValue2 = OptionValue2.concat(",").concat(OptionValue0);
			  }
			  else{
			  OptionValue1 = OptionValue;		
			  OptionValue2 = OptionValue0;		
			  }
   }
   }
   
  // OptionValue1=OptionValue1.replace("'","''");
   if(text0=="Claim Status"){
   elOptNew.value = value0+" in ( '"+OptionValue1+"' )";
  elOptNew.text = text0+seperator+OptionValue1;
   }
   else
   {
    //(OptionValue2);
  elOptNew.value = value0+" in ( '"+OptionValue2+"' )";
  elOptNew.text = text0+seperator+OptionValue1;
  }
   }
  var elSel = document.getElementById('final_this_page_criteria');
  TheOptionIndex=checkOptionExist2(value0," in ");
  ...
  if(TheOptionIndex==-1)
   ...
   {
   //  elOptNew.text = text0+seperator+text1;
    elSel.options[TheOptionIndex].value = value0+" in ( '"+OptionValue2+"' )";
	elSel.options[TheOptionIndex].text =  text0+seperator+OptionValue1;
    //elOptNew.value = value0+" in ( '"+OptionValue1+"' )";
    //elOptNew.text = text0+seperator+OptionValue1;
     // Sai custom code end
      }
...
function ProcessBeforeSubmitting()//Validations and necessary actions are taken here.
 ...
 // Sai custom code start
//alert(document.getElementById('final_this_page_criteria').options[0].text);


var boxes = document.getElementById('final_this_page_criteria');
 anyFilled=false;
    ...
    if(anyFilled)
	{}else{  alert ('Please set Current Criteria'); return false;}
