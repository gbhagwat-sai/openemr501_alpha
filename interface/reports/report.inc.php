...
function BuildArrayForReport($Query,$index)
 {//Parses the database value and prepares for display.
  ...
  while($row=sqlFetchArray($res))
   ...
   //if($index==13)
	...
	//else
	//$array_data[$row['status']]=htmlspecialchars($row['status'],ENT_QUOTES);
   }
  return $array_data;
 }
 //Gangeya Function tobuild array for status

 function BuildArrayForStatReport($Query)
 {//Parses the database value and prepares for display.
  $array_data=array();
  $res = sqlStatement($Query);
  while($row=sqlFetchArray($res))
   {
    $array_data[$row['id']]=htmlspecialchars($row['status'],ENT_QUOTES);
   }
  return $array_data;
 }
  // Sai custom code end
