...
<span class="title"><?php echo xlt('Edit User'); ?></span>&nbsp;
    ...
    <a class="btn btn-default btn-save" name='form_save' id='form_save' href='#' onclick='submitform();'> <span><?php echo xlt('Save');?></span> </a>
...
<TR>
    ...
    <!-- Added new code to get user email 
    Added by kiran  19-1-2017 -->
      ...
      <td  style="width:220px;">
        <?php if(isset($iter["email"]) && !empty($iter["email"])) {  ?>
          <span><?php echo $iter["email"]; ?></span>
		  <input type="hidden" id="email" name="email" value="<?php echo $iter["email"]; ?>" style="width:120px;">
        <?php }else {  ?>
          <input type="entry" id="email" name="email" value="<?php echo $iter["email"]; ?>" style="width:120px;">
        <?php }  ?>
        <span class="mandatory">&nbsp;*</span>
      </td>
      <td style="width:150px;">
        <span class="text"><?php xl('Servicing Provider','e'); ?>: </span>
      </td>
      <td border = '1' style="width:150px;">&nbsp; <input type="checkbox" name="servicingprovider" 
		<?php if ($iter["isServicingProvider"]) echo " checked"; ?> /></td>
      </tr>
      <!-- end -->
    <?php if (!$GLOBALS['use_active_directory']) { ?>
<TR>
...
<!--Added By gangeya 10758-->
        <td>
			<span class=text><?php xl('Term Date','e'); ?>: </span>
        </td>
        <td>
        	<input type='text' name='form_term_date' id="form_term_date" size='10' value='<?php echo $iter["term_date"]; ?>'
...
function submitform() { 
  ...
  // added validation by kiran for new email feild.
    if(trimAll(document.getElementById('email').value) == ""){
      document.forms[0].email.style.backgroundColor="red";
      alert("<?php xl('Required field missing: Please enter the valid email','e');?>");
...
function trimAll(sString)
  {
    ...
    {
      sString = sString.substring(0,sString.length-1);b
    }
