...
// code for single version
if(isset($_POST['loginsubmit']) && isset($_POST['clientname']))
{
    
    $c =$_POST['clientname'];
    $clientname= strtolower($c);
    $_GET["site_id"] = $clientname;
    $_SESSION["site_id"] = $clientname;
    ...
    <div class="container">
        <form method="POST" id="login_form"
         ...
         <div class="row">
                <div class="col-sm-12">
                    <div>
                        <div class="center-block" style="max-width:400px">
                            <img class="img-responsive center-block" src="<?php echo $GLOBALS['images_static_relative']; ?>/logo-payehr.gif" />
                        </div>
            ...
            $resAMA = sqlStatement("select * from users where username = '".$username."'");
            $row = sqlFetchArray($resAMA);

            $isAMAAgreed = $row['is_agreed'];
            ?>
              <div class="col-sm-12 text-center">
                  <p class="">
                    Confirm Connection to <strong><?php echo $c?></strong>
                  </p>
              </div>
        ...
        ?>
            <div class="form-group text-center">
             <input type="checkbox" id="amaAgreement" name="amaAgreement" value="check" onChange="document.getElementById('yes').disabled = !this.checked;"/> 
            
            I agree to the <a onClick="popup('agreement.html', 'EUA')">AMA End User agreement</a>
           
        <?php }?>
            </div>
         <input type="hidden" size="10" id="authUser" name="authUser" value="<?php echo $username;?>">
           <input type="hidden" size="10" id="clearPass" name="clearPass" value="<?php echo $password;?>">

       <!-- <input id="yes" type="submit" <?php if($isAMAAgreed == 0)echo 'disabled'; else '';?> onClick="chk_hash_fn();" value=<?php xl('Yes','e');?>>-->
        <div class="form-group text-center">
                        <button type="submit" class="btn btn-default btn-lg" onClick="transmit_form()"><i class="fa fa-sign-in"></i>&nbsp;&nbsp;<?php echo xlt('Yes');?></button>
                        <a class="btn btn-default btn-lg" href="../../user_login.php?user=<?php echo $username ?>&client=<?php echo $clientname ?>" ><strong>Return</strong></a> 
        </div>
