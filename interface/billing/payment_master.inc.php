...
function generate_list_payment_category(
$tag_name,
$list_id,
$currvalue,
$title,
...
if ($payment_id>0) {
    ...
    if ($row['payment_type']=='insurance' || $row['payer_id']*1 > 0) {
        ...
        $res = sqlStatement("SELECT fname,lname,mname FROM patient_data
				where pstatus = 'YES' and pid ='$PatientId'"); // Sai custom code
        $row = sqlFetchArray($res);
            $fname=$row['fname'];
            $lname=$row['lname'];
            $mname=$row['mname'];
	  ...
	  // Sai custom code start
	 	$res = sqlStatement("select file from eob_file where session='$payment_id'");
		$row = sqlFetchArray($res);
		$file_name = $row['file'];
	 // Sai custom end
}
?>
<script type="text/javascript" src="<?php echo $GLOBALS['webroot'] ?>/library/textformat.js"></script><!--   Sai custom code -->
<?php
//================================================================================================
...
<!--  Sai custom code end -->
        <td align="left" class="text" ><input type='text' size='9' class='datepicker' name='check_date' id='check_date' class="class1 text " readonly="readonly" value="<?php echo htmlspecialchars(oeFormatShortDate($CheckDate));?>"/></td><!--   Sai custom code -->
        <td> </td>
	<!--   Sai custom code  start-->
		<td><?php /*?><img src='../../interface/main/calendar/modules/PostCalendar/pntemplates/default/images/new.jpg' align='absbottom'
      ...
      <tr height="1">
	  <!--   Sai custom code  start-->
		<td colspan="11" align="left" class="text" style="padding-left:5px;" ><font size="1" color="#FF0000"> Insurance Search Criteria ( Code / name / CMS/Payer ID ) <br>  Patient Search Criteria ( Code / Name / DOB(yyyy/mm/dd) / SSN(nnn-nn-nnnn) )</font></td>        
         <td  colspan="3" align="center"  class="text"><label for="file">EOB File: </label><input type="hidden" name="MAX_FILE_SIZE" value="5000000" />
   <input name="form_eobfile" id="uploadedfile"  type="file" class="text" size="10" style="display:inline" />
        </td>
	<!--   Sai custom code  end-->
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td align="left"><input type='text' size='9' class='datepicker' name='deposit_date' id='deposit_date'  onKeyDown="PreventIt(event)"   class="class1 text " value="<?php echo htmlspecialchars(oeFormatShortDate($DepositDate));?>"   onkeyup='datekeyup(this)' onBlur="dateblur(this,mypcc,'<?php echo date("m/d/Y"); ?>')"  /><!-- Sai custom code -->
      ...
      <tr>
        ...
        <table border="0" cellspacing="0" cellpadding="0" width="280">
          ...
          <tr>
		    ...
		    <td width="70"><a href="#" class="css_button" onClick="javascript:OpenEOBEntry();"><span><?php echo htmlspecialchars( xl('Apply'), ENT_QUOTES);?></span></a></td><!--   Sai custom code -->
          </tr>
        </table>        </td>
      </tr>
    <?php
        ...
        <td colspan="13" align="left" >
            ...
            <table width="969" border="0"  cellspacing="0" cellpadding="0" align="left">
				  ...
				  <!--   Sai custom code start-->
                    ...
                    <td class="text" align="left"><?php echo htmlspecialchars( xl('EOB File'), ENT_QUOTES).':' ?></td>
                    <td  colspan="3" align="left"  class="text bottom left top right">&nbsp;<?php //generate_print_field(); 					
					echo  htmlspecialchars($file_name);  ?>
                    <input type="hidden" name="form_eobfile" value="<?php echo htmlspecialchars($EOBFileName);?>"/>
        </td>
			<!--   Sai custom end -->
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td align="left" class='text'></td>
                    <td align="left" class='text'><?php echo htmlspecialchars(xl('Deposit Date'), ENT_QUOTES).':' ?></td>
                    <td align="left" class="text bottom left top right">&nbsp;<?php echo htmlspecialchars(oeFormatShortDate($DepositDate));?><input type="hidden" name="deposit_date" value="<?php echo htmlspecialchars(oeFormatShortDate($DepositDate));?>"onkeyup='datekeyup(this)' onBlur="dateblur(this,mypcc,'<?php echo date("m/d/Y"); ?>')"  />	   </td><!--   Sai custom code -->
                    <td></td>
                    <td align="left" class="text"><?php echo htmlspecialchars(xl('Description'), ENT_QUOTES).':' ?></td>
