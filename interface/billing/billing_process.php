...
// code change for BUG ID 10643
if (isset($_POST['form_with']) && $_POST['form_with']=='1') 
$bat_filename = date("Y-m-d-Hi", $bat_time) . "-batch.";
else
    $bat_filename = date("Y-m-d-Hi", $bat_time) . "-batch-preview.";
//sai custom code end
...
function process_form($ar)
{
    ...
    $billFlag=0; // code added for additional invoices //sai custom code 
        $ta = explode("-", $claimid);
        $patient_id = $ta[0];
        $encounter = $ta[1];
...
//sai custom code  start
      // code for additional invoices to get encounter Date
      $sqldos = "select date as dos from form_encounter where encounter='" .$encounter. "'";
      $encounterObject = $db->Execute($sqldos);
      $encounterDOS = $encounterObject->fields['dos'];
      //echo " encounter dos===".$encounterDOS;

      // code added for Additional PHP invoices 
      $PHPdate = '2017-04-01 00:00:00';
      if(strtotime($encounterDOS) >= strtotime($PHPdate)){
          $billFlag =1; 
      } 

        $sql1 = "select min(date) as dt from form_encounter where pid='" .$patient_id. "'";
        $FirstEncDate = $db->Execute($sql1);
        $FirstEncDate = date('m/d/Y', strtotime($FirstEncDate->fields['dt']));
        //Changes by sangram babar 
        //$FirstEncDate is used to display date in box 12 in hcfa
//sai custom code  end
        if (isset($claim_array['bill'])) {
            ...
            if (isset($ar['HiddenMarkAsCleared']) && $ar['HiddenMarkAsCleared'] == 'yes') {
             //sai custom code  start           
            if (isset($ar['form_with']) && $ar['form_with']=='1') 
                    $tmp = updateClaim(true, $patient_id, $encounter, $payer_id, $payer_type, 2); // $sql .= " billed = 1, ";
            else
                $tmp = updateClaim(true, $patient_id, $encounter, $payer_id, $payer_type, 2,-1,'','',-1,0,0);
        // Sai custom code end
            }
            ...
            if (! $tmp) {
                ...
                if (isset($ar['bn_mark'])) {
                    ...
                    $log = '';
                    $lines = gen_hcfa_1500($FirstEncDate,$patient_id, $encounter, $log,$version); //sai custom code  start
                    fwrite($hlog, $log);
                    ...
                    foreach ($alines as $tmplines) {
                      ...
                      // code change for BUG 10643
                       if (isset($ar['form_with']) && $ar['form_with']=='1') {
                        ...
                        // code change for BUG ID 10643
                      if (isset($_POST['form_with']) && $_POST['form_with']=='1') 
                        UpdateEncounterStatus($encounter);
                    ...
                    $version='new'; //sai custom code
                    $log = '';
                    $lines = gen_hcfa_1500($FirstEncDate,$patient_id, $encounter, $log,$version);
                    fwrite($hlog, $log);
                    $bat_content .= $lines;
                   // echo $bat_content;
                    if (isset($ar['form_with']) && $ar['form_with']=='1') {
                ...
                //updated by sangram - function for updating status automatically
                         UpdateEncounterStatus($encounter); // Sai custom code
                    }
    ...
    //sai custom code start
        // code added by pawan for storing cms file page count
        if (isset($ar['form_with']) && $ar['form_with']=='1') {
            
            $CMSFileName = $bat_filename;
        $CMSFilePath= $GLOBALS['OE_SITE_DIR'] . "/edi/".$CMSFileName;
        $CMSPageCount =getNumPagesPdf($CMSFilePath);
        //echo "====********==".$c; 
        updateCMSPageCount($CMSFileName,$CMSPageCount,$billFlag);
...
//sai custom code start 
// function for getting number of pages in pdf
