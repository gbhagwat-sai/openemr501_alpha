...
function select_all() {
  for($i=0;$i < document.update_form.length;$i++) {
	...
	$checkboxstatus = document.update_form[$i].disabled; // Sai custom code
    if ($name.substring(0,7) == "claims[" && $name.substring($name.length -6) == "[bill]" && $checkboxstatus == false) { // Sai custom code	
...
function check_bill_status(ins_status,check_status)
{
if(check_status==0)
alert("Claim is not Ready to send");
if(ins_status==0)
alert("Insurance Company not assigned.");
return false;
}
// Sai custom code end
// Process a click to go to an encounter.
function toencounter(pid, pubpid, pname, enc, datestr, dobstr,note) { // Sai custom code
 top.restoreSession();
 encurl = 'patient_file/encounter/encounter_top.php?set_encounter=' + enc + '&pid=' + pid;
 parent.left_nav.setPatient(pname,pid,pubpid,'',dobstr,note); // Sai custom code
...
function topatient(pid, pubpid, pname, enc, datestr, dobstr,note) { // Sai custom code
 top.restoreSession();
...
{
 	// code added for BUG 10686
 	alert("Please select the checkbox WITH UPDATE if you want to change billdate.");// Sai custom code
...
if ($daysheet) {
    $ThisPageSearchCriteriaDisplayMaster = array(
        ...
        xl("X12 Partner"),
        xl("User"),
	xl("CPT4 Or ICD9/ICD10"), // Sai custom code
	xl("Bill Amount"), // Sai custom code
	xl("Encounter Status") ); // Sai custom code
    ...
    $ThisPageSearchCriteriaDataTypeMaster = "datetime,datetime,datetime,radio,text_like," .
        ...
        "radio_like,radio,query_drop_down,text,text,query_drop_down";//Sai custom code
} else {
    $ThisPageSearchCriteriaDisplayMaster = array(
        xl("Date of Service"),
        xl("Date of Entry"),
...
//Sai custom code start
//Gangeya Added for Encounter Status
$ThisPageSearchCriteriaQueryDropDownMaster[2]="SELECT status as name,id  FROM claim_status;";
...
if (! isset($_REQUEST['mode'])) { // default case
    ...
    $_REQUEST['final_this_page_criteria'][2] = "form_encounter.encounter = '0'";
	...
	$_REQUEST['final_this_page_criteria_text'][2] = xl("Encounter = 0");
...
<!-- Criteria section Ends -->
	  ...
	  <!-- sai custom code start -->
          <!-- code added for BUG ID 8802 patient statement-->
	<tr>
            <td>&nbsp;</td>
          <td><?php  print '<span class=text><a  class="btn btn-default btn-sm" href="javascript:void window.open(\'sl_eob_search_patient.php\')" onclick="top.restoreSession()">' . htmlspecialchars(xl('Patient Statement'), ENT_QUOTES) . '</a></span>'; ?></td>
         </tr>
			<tr>
              <td>&nbsp;</td>
            <td><?php  print '<span class=text><a  class="btn btn-default btn-sm" href="javascript:void window.open(\'statement_search.php\')" onclick="top.restoreSession()">' . htmlspecialchars(xl('View Printed Statement'), ENT_QUOTES) . '</a></span>'; ?>
            </td> 
            </tr>
	<!-- Sai custom coode end -->	     
...
<?php } ?>
<!-- Sai custom code line -->
<input type='checkbox' name='form_with' value='1' /><font color="#0000FF" weight="bold"> <?php xl('With Update','e'); ?></font>
...
<?php
    ...
    foreach ($ret as $iter) {
      ...
      $name = getPatientData($iter['enc_pid'], "fname, mname, lname, pubpid, DATE_FORMAT(DOB,'%Y-%m-%d') as DOB_YMD,Alert_note");// sai custom code
	     ...
	     //Sai custom code start
		$St=CheckEncounterStatus($iter['enc_encounter']);

		//print 'alert("Status'. $St["Status"] .' ..")';

		if(stripos($St["Status"],"Ready to send")===0) 
		$chk_status=1;
		else
		$chk_status=0;
                $lcount += 2;
		$lhtml .= "<br /><select name='EncounterNo[" . $iter['enc_encounter'] . "]' id='EncounterNo[" . $iter['enc_encounter'] . "]' style='background-color:$bgcolor' disabled>";
	 $lhtml .= "<option value=".$St['Status']." $selected1 >".$St["Status"]."</option>";
		$lhtml .= "</select> \n";
		// Sai custom cod end
                ...
                $prevtype = '';
		$chk_ins_cmp = 0; // Sai custom code line
        ...
        if ($iter['id'] && $last_encounter_id != $this_encounter_id) {
		 ...
		 //Sai custom code   start
		  if($chk_ins_cmp ==1 && $chk_status ==1)
		  $disabled = "";
		  else
		  $disabled="disabled";
            
	    $rhtml .= "<td><div onclick='check_bill_status($chk_ins_cmp,$chk_status)'><input type='checkbox' value='$tmpbpr' name='claims[" . $this_encounter_id . "][bill]' onclick='set_button_states()' id='CheckBoxBilling" . $CheckBoxBilling*1 . "' $disabled >&nbsp;</div></td>\n";	
