...
// Sai custom code start		
	...
	/* start file upload*/
	if ($_FILES["form_eobfile"]["error"] > 0)
    	{
    		//echo "Return Code: " . $_FILES["form_eobfile"]["error"] . "<br>";
    	}
  	else
    	{ 
		if ( ! is_dir($GLOBALS['OE_SITE_DIR'].'/eob/'.$payment_id.'/')) {
			mkdir($GLOBALS['OE_SITE_DIR'].'/eob/'.$payment_id.'/');
		}    
     		$filepath = $GLOBALS['OE_SITE_DIR'].'/eob/'.$payment_id.'/';
		
		if (file_exists("$filepath" . $_FILES["form_eobfile"]["name"]))
		{
			echo $_FILES["form_eobfile"]["name"] . " already exists. ";
		}
		else
		{
			move_uploaded_file($_FILES["form_eobfile"]["tmp_name"],"$filepath" . $_FILES["form_eobfile"]["name"]);     
		  	//echo "Stored in: " . "$filepath" . $_FILES["form_eobfile"]["name"];
		}
    	}
	/* end file upload*/
		$eob_id = idSqlStatement("insert into eob_file set session='".$payment_id."', file='".trim($_FILES["form_eobfile"]["name"])."' ");
 // Sai custom code end
}
...
if ($mode == "PostPayments" || $mode == "FinishPayments") {
	$hidden_patient_code=$_REQUEST['hidden_patient_code'];// Sai custom code
	$payment_id=$_REQUEST['payment_id'];// Sai custom code
    $user_id=$_SESSION['authUserID'];
...
//Checks whether any of the allocation row is filled.
	...
	for(RowCount=1;;RowCount++){
		...
		if(!document.getElementById('Payment'+RowCount))
			...
			if(document.getElementById('Allowed'+RowCount).value.length!='0')
			{
				if(new Date(document.getElementById('payment_deposit_date').value) < new Date(document.getElementById('HiddenEncounterDOS'+RowCount).value))
				{
					alert('Encounter :' + document.getElementById('HiddenEncounter'+RowCount).value + '   Payment deposit date should be greater than Encounter date of service');
					return true;
				}
			}
		}
  ...
  for(RowCount=1;;RowCount++)
   {
	 ...
	 if(document.getElementById('Allowed'+RowCount).value=='' && document.getElementById('Payment'+RowCount).value=='' && document.getElementById('AdjAmount'+RowCount).value=='' && document.getElementById('Deductible'+RowCount).value=='' && document.getElementById('Takeback'+RowCount).value=='' && document.getElementById('FollowUp'+RowCount).checked==false && document.getElementById('payment_ins'+RowCount).value!=0)
            {

            }
	    else if(document.getElementById('HiddenCopayAmount'+RowCount)){		
	    
	    	if(document.getElementById('payment_ins'+RowCount).value==0 && document.getElementById('HiddenCopayAmount'+RowCount).value==0 && document.getElementById('Payment'+RowCount).value==''){
  ...
  if(after_value=='distribute')
  ...
  else if(after_value=='new_payment')
   {
    if(document.getElementById('TablePatientPortion'))
     ...
     // sai custom code
     if(confirm("<?php echo htmlspecialchars( xl('Successfully Saved.Would you like to Apply?'), ENT_QUOTES) ?>"))
     {
        if(document.getElementById('TablePatientPortion'))
         {
...
<?php
    ...
    ?>
' style="display:inline" enctype="multipart/form-data" > <!-- Sai custom code -->
...
<!-- Sai custom code -->
<input type="hidden" name="payment_deposit_date" id="payment_deposit_date" value="<?php echo htmlspecialchars($payment_deposit_date);?>"/>
<input type="hidden" name="hidden_type_code" id="hidden_type_code" value="<?php echo htmlspecialchars($hidden_type_code);?>"/>
<?php /*?>commented for bug 8904<?php */?>
