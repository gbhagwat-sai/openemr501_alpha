...
function writeoff(code) {
 ...
 var welement = f['form_line[' + code + '][w_o]']; // Sai custom code
 var tmp = belement.value - (pelement.value + welement.value); // sai custom code
...
if (! $trans_id) {
    ...
    if ($_POST['form_save'] || $_POST['form_cancel']) {
		...
		// Sai custom code end              // The sl_eob_search page needs its invoice links modified to invoke
        ...
        if ($ALLOW_DELETE && !$debug && isset($_POST['form_del'])) { // sai custom code
...
<?php xl('Svc Date:', 'e'); ?>
  ...
  <td>
<?php
    echo preg_replace("/(\d+)\D+(\d+)\D+(\d+)/","$2/$3/$1",$svcdate); // Sai custom code
?>
  </td>
  <td colspan="2">
...
?>
 ...
 <tr>
  ...
  <td>
    <?php
    echo preg_replace("/(\d+)\D+(\d+)\D+(\d+)/","$2/$3/$1",$billdate); // Sai custom code
?>
  </td>
  <td colspan="2">
...
<table border='1' cellpadding='2' cellspacing='0' width='98%'> <!-- Sai custom code -->
...
foreach ($codes as $code => $cdata) {
   ...
   // this sorts the details more or less chronologically:
    ...
    foreach ($cdata['dtl'] as $dkey => $ddata) {
        ...
        $tmpchg = "";
        $tmpadj = "";
	$tmpwo = ""; // sai custom code
	$bal1 = ""; // sai custom code
      /*****************************************************************
     if ($ddata['chg'] > 0)
      $tmpchg = $ddata['chg'];
     else if ($ddata['chg'] < 0)
 ...
 // Sai custom code start
 // Code changes by Mahesh. updated code to get currect wr_off balance
//	if($ddata['chg'])
//	$tmpwo = $cdata['w_o'];
	...
	$tmpwo = $ddata['wr_off'];
// code changes end. 	
	$amt_arr1 = explode("$",$ddata['rsn']);
	if($amt_arr[1]) $amt_value = $amt_arr1[1];
	else
	{
	$amt_arr2 = explode(": ",$ddata['rsn']);
	$amt_value = $amt_arr2[1];
	}
 	 if($tmpchg!='' || $ddata['plv'] ||  $ddata['src'] || $ddata['pmt'] || $tmpadj || $ddata['wr_off'] || $amt_value!=0){
// Sai custom code end
    ?>
  ...
  <td class="detail">
   ...
   <td class="detail">
   <?php echo preg_replace("/(\d+)\D+(\d+)\D+(\d+)/","$2/$3/$1",$ddate) ?><!-- Sai custom code -->
   </td>
   <td class="detail">
    <?php bucks($ddata['pmt']) ?>
   </td>
  ...
  <td class="detail">
   <?php bucks($tmpwo) ?> <!-- Sai custom code -->
   </td>
   <td class="detail">
    <?php echo $ddata['rsn'] ?>
   </td>
...
<?php printf("%.2f", ($cdata['bal']-$cdata['w_o']));// Sai custom code?> &nbsp;
</td>
<td class="detail">
...
style="background-color:<?php echo $bgcolor ?>" />
  ...
  <td class="detail">
     <input type="hidden" name="form_line[<?php echo $code ?>][w_o]" value="<?php echo $cdata['w_o'] ?>">
   <a href="" onClick="return writeoff('<?php echo $code ?>')"><font size="-1"> click for Adjustment</font></a>
  </td>
  <!-- Sai custom code  end-->
<td class="detail">
...
?>
 ...
 <td class="dehead" colspan="2">
   ...
   <table border='0' cellpadding="1"  class="text" width = "50%" align="left" class="flat-table">
		<tr>
   ...
   //$note_arr = str_replace(":[","<br>[",$cdata['note']);
   	//echo $note_arr;
		//echo "pawan";
		echo "<tr>";
		echo "<td>$date</td>";
