...
if (isset($_POST["mode"])) {
    ...
    //===============================================================================
        ...
        for ($CountRow=1; $CountRow<=$CountIndexAbove; $CountRow++) {
		  ...
		  if (isset($_POST["Payment$CountRow"]) && strlen($_POST["Payment$CountRow"])>0 && $_POST["Payment$CountRow"]*1>=0) // Sai custom code
                    if (trim(formData('type_name'))=='insurance') {
			      ...
			      "' where  session_id ='$payment_id' and pid ='" . trim(formData("HiddenPId$CountRow"   ))  .
                              ...
                              "' and  code  ='" . trim(formData("HiddenCode$CountRow"))  .
                              "' and  modifier  ='" . trim(formData("HiddenModifier$CountRow"))  .
			      //"' and  Adj_code = '" . trim(formData("Adj_code$CountRow"   ))  . // sai custom code
                              "' and pay_amount>0");
                    } else {
		    		// Sai custom code
				 if(trim(formData("Payment$CountRow"))>0) {	
                                 sqlBeginTrans();
	   ...
	   if($copay_amount != $post_copay_amount && $copay_amount==0){
		...
		sqlStatement("UPDATE ar_activity set pay_amount= '".trim(formData("HiddenCopayAmount$CountRow"))."', modified_time = '". trim($created_time) ."', post_time = '" . trim($created_time) ."', post_user = '" . trim($user_id)."',memo = '"."$AdjustString"  ."' where pid='". trim(formData('hidden_patient_code' ))."' and  encounter = '" . trim(formData("HiddenEncounter$CountRow"))."' and session_id='"    . trim(formData('payment_id')) ."' and code='" . trim(formData("HiddenCode$CountRow" ))  ."' and account_code='PCP'   ");
	     ...
	     // Sai custom code end
                ...
                if (isset($_POST["AdjAmount$CountRow"]) && $_POST["AdjAmount$CountRow"]*1!=0) {
                    ...
                    if (sqlNumRows($resPayment)>0) {
                              ...
                              sqlStatement("insert into ar_activity set "    .
                                ...
                                "', memo = '" . "$AdjustString"  .
                                "', account_code = '" . "$AccountCode"  .
				"', Adj_code = '" . trim(formData("Adj_code$CountRow"   ))  . // sai custom code
                                "'");
                                sqlCommitTrans();
                    }
                } else {
          ...
          //==============================================================================================================================
                if (isset($_POST["Deductible$CountRow"]) && $_POST["Deductible$CountRow"]*1>0) {
                      $resPayment = sqlStatement("SELECT  * from ar_activity " .
                          ...
                          "' and  code  ='" . trim(formData("HiddenCode$CountRow"))  .
                          "' and  modifier  ='" . trim(formData("HiddenModifier$CountRow"))  .
			  "' and (memo like 'Deductable%' or memo like '%dedbl%')"); // sai custom code
          ...
          //==============================================================================================================================
// Sai custom code start
			if ($_POST["Payment$CountRow"]*1>=0 && ($_POST["wo$CountRow"]*1 >0 || $_POST["Allowed$CountRow"]*1 >0)) // code change for satisfy 0 payment BUG 10504
			{
				...
				if(($_POST["wo$CountRow"]*1 >0) || ($_POST["wo$CountRow"]*1 <0) && $_POST["wo$CountRow"]!='') {
					...
					$resPayment = sqlStatement("SELECT  * from ar_activity " .
						...
						"' and  code  ='" . trim(formData("HiddenCode$CountRow"   ))  .
						"' and  modifier  ='" . trim(formData("HiddenModifier$CountRow"   ))  .
						"' and (w_o!=0 || allowed_amt!=0) and pay_amount=0 and memo !=''");	// code change for BUG  10504				
					...
					if(sqlNumRows($resPayment)>0)
					{
						...
						"' and  code  ='" . trim(formData("HiddenCode$CountRow"   ))  .
						"' and  modifier  ='" . trim(formData("HiddenModifier$CountRow"   ))  .					
						"' and (w_o!=0 || allowed_amt!=0) and pay_amount=0 and reason_code='wo'");		// query change for double w/o	10504	
					}
					else
					{
						sqlStatement("insert into ar_activity set "    .
...
//==============================================================================================================================
	   ...
	   // Sai custom code  	commented for bug 8904
...
// Sai custom code
$ResultSearchSub = sqlStatement("SELECT distinct (ar.encounter),ar.code_type,ar.code,ar.modifier,ar.pid,pd.fname,pd.lname,pd.mname,pd.pubpid from ar_activity as ar left join patient_data as pd ON ar.pid = pd.pid where ar.session_id ='$payment_id' order by ar.pid,encounter,code,modifier");
...
//===============================================================================
 ...
 if (isset($_POST["mode"]))
 {
  ...
  echo '"' . xl('Interest') . '",';
  echo '"' . xl('Adj code') . '",';
  echo '"' . xl('Mannual Adj Code') . '",';
  echo '"' . xl('Follow Up Reason / Note') . '"' . "\n";
...
if($RowSearchSub = sqlFetchArray($ResultSearchSub))
 {
	do 
	 {
		...
		/*$ResultSearch = sqlStatement("SELECT billing.id,last_level_closed,billing.encounter,form_encounter.`date`,billing.code,billing.modifier,fee,units
		 ...
		 form_encounter.pid ='$PId' and billing.pid ='$PId' and billing.encounter ='$EncounterMaster'
		  and billing.code ='$CodeMaster' and billing.modifier = '$ModifierMaster'
		 ...
		 FROM billing ,form_encounter
		 where billing.encounter=form_encounter.encounter and billing.pid=form_encounter.pid and 
		 code_type='CPT4' and billing.activity!=0 and 
		 form_encounter.pid ='$PId' and billing.pid ='$PId' and billing.encounter ='$EncounterMaster'
		  and billing.code ='$CodeMaster' and billing.modifier = '$ModifierMaster'
		 ORDER BY form_encounter.`date`,form_encounter.encounter,billing.code,billing.modifier");
		 
		// modifier condition added for BUG 10750
		//  die;
			...
			while ($RowSearch = sqlFetchArray($ResultSearch))
			 {
				...
				if($BillingId!=$Id)//multiple cpt in single encounter
				 {
					$Copay=0.00;
					$editable_copay = ' type="hidden" ';
					
				 }
				else{
				$editable_copay =  ' style="width:45px;text-align:right; font-size:12px" type="text" ' ;
					...
					$Copay=$Copay+$PatientPay;
									...
									/*if($Ins==0)
									 {*///Got just before Patient
										$resMoneyGot = sqlStatement("SELECT sum(pay_amount) as MoneyGot,sum(adj_amount) as MoneyAdjusted,w_o FROM ar_activity where pid ='$PId' and  code='$Code' and modifier='$Modifier'  and  encounter  ='$Encounter' and  payer_type !=0");
										$rowMoneyGot = sqlFetchArray($resMoneyGot);
		...
		$resReceipt = sqlStatement("SELECT sum(pay_amount) as receipts FROM ar_activity where code='$Code' and modifier='$Modifier' and  encounter  ='$Encounter' ");
									...
									/* }
									 ...
									 {*///Got just before the previous
										...
										/*$resSequence = sqlStatement("SELECT  sequence_no from ar_activity where  session_id ='$payment_id' and pid ='$PId' and  encounter  ='$Encounter' order by sequence_no  ");
										$rowSequence = sqlFetchArray($resSequence);
										$Sequence=$rowSequence['sequence_no'];

										$resMoneyGot = sqlStatement("SELECT sum(pay_amount) as MoneyGot,sum(adj_amount) as MoneyAdjusted,w_o FROM ar_activity where pid ='$PId' and  code='$Code' and modifier='$Modifier'   and  encounter  ='$Encounter' 
									...
									$RemainderJS = $Fee - ($Copay + $MoneyGot + $MoneyAdjusted + $W_O);
										...
										$resPayment = sqlStatement("SELECT  pay_amount,memo,w_o,co_ins,interest,allowed_amt from ar_activity where  session_id ='$payment_id' and pid ='$PId' and  encounter  ='$Encounter' and  code='$Code' and modifier='$Modifier'  and pay_amount>0 and account_code!='PCP' ");
  ...
  $RemainderJS += $claim_payment_row['pay_amount'];
	...
	$cia = sqlStatement("select co_ins as co_ins  from ar_activity where encounter='$Encounter' and  code='$Code' and modifier='$Modifier' and co_ins>0  order by post_time desc limit 1");
   while ($ciar = sqlFetchArray($cia)) {
     $co_ins=$ciar['co_ins'];
   }
   
   	    //below added by Sangram for  Bug 9117  
  $ia = sqlStatement("select sum(interest) as interest  from ar_activity where encounter='$Encounter' and  code='$Code' and modifier='$Modifier' and interest>0 and pay_amount>0 ");
   while ($iar = sqlFetchArray($ia)) {
     $interest=$iar['interest'];
   }
									 ...
									 //$allowedtot=$allowed_amt1+$allowedtot;
												    ...
												    //coins and interest are added by Sangram for  Bug 9117   
								
						echo '"' . addslashes($NameDB) . '",';  
						echo '"' . addslashes($PId) . '",';   
						echo '"' . addslashes($Ins) . '",'; 
						echo '"' . addslashes($ServiceDate) . '",'; 
						echo '"' . addslashes($Encounter) . '",'; 
						echo '"' . addslashes($Code) . '",'; 	
						echo '"' . addslashes($Fee) . '",'; 
						echo '"' . addslashes($Copay) . '",'; 
						echo '"' . addslashes(round($RemainderJS,2)) . '",'; 	
						echo '"' . addslashes($allowed_amt) . '",';	
                      	echo '"' . addslashes($PaymentDB) . '",';
                       	echo '"' . addslashes($AdjAmountDB) . '",';     
						echo '"' . addslashes($DeductibleDB) . '",';    
						echo '"' . addslashes($TakebackDB) . '",'; 	
						echo '"' . addslashes($W_O) . '",'; 
						echo '"' . addslashes($co_ins) . '",'; 
						echo '"' . addslashes($interest) . '",'; 	
						echo '"' . addslashes($adj['Adj_code']) . '",'; 	
						echo '"' . addslashes($ReasonCodeDB) . '",'; 	
						echo '"' . addslashes($FollowUpReasonDB). '"' . "\n";
				}//while ($RowSearch = sqlFetchArray($ResultSearch))
				}//if(sqlNumRows($ResultSearch)>0)
...
function ModifyPayments()
 ...
 function ExportToCSV()
 {
	...
	document.getElementById('mode').value='ExportToCSV';
	clearpaymentFields();
	document.forms[0].submit();
 return true;
 }
        ...
        if(confirm("<?php echo htmlspecialchars(xl('Would you like to Modify and Finish Payments?'), ENT_QUOTES) ?>"))
         {
            UnappliedAmount=document.getElementById('TdUnappliedAmount').innerHTML*1;
			 // Sai custom code start
			<?php /*?>if(confirm("<?php echo htmlspecialchars( xl('Undistributed is'), ENT_QUOTES) ?>" + ' ' + UnappliedAmount + '.' + "<?php echo htmlspecialchars('\n');echo htmlspecialchars( xl('Would you like the balance amount to apply to Global Account?'), ENT_QUOTES) ?>"))
			 {
...
function OnloadAction()
 ...
 hideShowtable("true");  // Sai custom code
...
function DeletePaymentDistribution(DeleteId)
 {//Confirms deletion of payment distribution.
  ...
  // Sai custom code start
 function hideShowtable(hide) 
 { 

 hide = document.getElementById("PaymentDistributePortion").style.display;
	if (hide=="none") {
	document.getElementById("PaymentDistributePortion").style.display="table";
	//document.getElementById("loginLink").onclick = hideShowtable(false);
	} else {
	document.getElementById("PaymentDistributePortion").style.display="none";
	//document.getElementById("loginLink").onclick = hideShowtable(true);
	}	
 }
 
//========================================================================================
/* FOLLING CODE IS ADDED FOR DISPLYING HIDE/SHOW PAYMENT LIST*/ 
...
<table width="1024" border="0" cellspacing="0" cellpadding="10" bgcolor="#DEDEDE"><tr><td>
           ...
           <td colspan="13" align="left" >
                ...
                if ($RowSearchSub = sqlFetchArray($ResultSearchSub)) {
                    do {
                        ...
                        $CodeMaster=$RowSearchSub['code'];
                        $ModifierMaster=$RowSearchSub['modifier'];
                        $res = sqlStatement("SELECT fname,lname,mname, pubpid FROM patient_data	where pid ='$PId'");
                        $row = sqlFetchArray($res);
                        $fname=$row['fname'];
                        $lname=$row['lname'];
                        $mname=$row['mname'];
                        $NameDB=$lname.' '.$fname.' '.$mname;
                        $ExternalID = $row['pubpid'];
                        ...
                        if (sqlNumRows($ResultSearch)>0) {
                            if ($CountPatient==1) {
                                 $Table='yes';
                                ?>
                            <table width="1300"  border="0" cellpadding="0" cellspacing="0" align="center" id="TableDistributePortion">
                            ...
                            <td width="70" class="left top" ><?php echo htmlspecialchars(xl('Post For'), ENT_QUOTES) ?></td>
                            <td width="70" class="left top" ><?php echo htmlspecialchars(xl('External ID'), ENT_QUOTES) ?></td>
	...
	<td align="left" class="<?php echo $StringClass; ?>" ><?php echo htmlspecialchars($NameDB); ?><input name="HiddenPId<?php echo $CountIndex; ?>" id="HiddenPId<?php echo $CountIndex; ?>" value="<?php echo htmlspecialchars($PId); ?>" type="hidden"/></td>
	...
	<td class="<?php echo $StringClass; ?>" ><?php echo htmlspecialchars($ExternalID); ?></td>
    <td class="<?php echo $StringClass; ?>" ><?php echo htmlspecialchars($ServiceDate); ?></td>
	<td align="right" class="<?php echo $StringClass; ?>" ><input name="HiddenEncounter<?php echo $CountIndex; ?>"  id="HiddenEncounter<?php echo $CountIndex; ?>"  value="<?php echo htmlspecialchars($Encounter); ?>" type="hidden"/><?php echo htmlspecialchars($Encounter); ?></td>
	<td class="<?php echo $StringClass; ?>" ><input name="HiddenCodetype<?php echo $CountIndex; ?>" id="HiddenCodetype<?php echo $CountIndex; ?>" value="<?php echo htmlspecialchars($Codetype); ?>" type="hidden"/><input name="HiddenCode<?php echo $CountIndex; ?>" id="HiddenCode<?php echo $CountIndex; ?>" value="<?php echo htmlspecialchars($Code); ?>" type="hidden"/><?php echo htmlspecialchars($Code.$ModifierString); ?><input name="HiddenModifier<?php echo $CountIndex; ?>"  id="HiddenModifier<?php echo $CountIndex; ?>" value="<?php echo htmlspecialchars($Modifier); ?>" type="hidden"/></td>
	...
	<td class="<?php echo $StringClass; ?>" ><input  type="text" name="Payment<?php echo $CountIndex; ?>" id="Payment<?php echo $CountIndex; ?>" onKeyDown="PreventIt(event)"   autocomplete="off"  id="Payment<?php echo $CountIndex; ?>" value="<?php echo htmlspecialchars($PaymentDB); ?>"  onChange="ValidateNumeric(this);ScreenAdjustment(this,<?php echo $CountIndex; ?>);UpdateTotalValues(1,<?php echo $TotalRows; ?>,'Payment','paymenttotal');RestoreValues(<?php echo $CountIndex; ?>)"  style="width:60px;text-align:right; font-size:12px" /></td>
	...
	<td class="<?php echo $StringClass; ?>" ><input  name="Takeback<?php echo $CountIndex; ?>"  id="Takeback<?php echo $CountIndex; ?>"  onKeyDown="PreventIt(event)"   autocomplete="off"   id="Takeback<?php echo $CountIndex; ?>"   value="<?php echo htmlspecialchars($TakebackDB); ?>"   onChange="ValidateNumeric(this);ScreenAdjustment(this,<?php echo $CountIndex; ?>);UpdateTotalValues(1,<?php echo $TotalRows; ?>,'Takeback','takebacktotal');RestoreValues(<?php echo $CountIndex; ?>)"   type="text"   style="width:60px;text-align:right; font-size:12px" /></td>
          ...
          <tr>
            <td colspan="13" align="left" >
                <table border="0" cellspacing="0" cellpadding="0" width="217" align="center">
                 ...
                 <tr>
	<td width="200"><a href="#" onClick="javascript:return ExportToCSV();"  class="css_button"><span style="font-size: 10px; font-weight:bolder"><?php echo htmlspecialchars( xl('Export CSV'), ENT_QUOTES);?></span></a>
	</td>
	<!-- code added by pawan for hide show -->
	<td width="200"><a href="#" onClick='javascript:void(0);return hideShowPaymentList("<?php echo $payment_id; ?>");' class="css_button"> <span style="font-size: 10px; font-weight:bolder"><?php echo htmlspecialchars( xl('Hide/Show Payment'), ENT_QUOTES);?></span></a>
...
<?php /*?>commented for bug 8904 sai custom code<?php */?>
<?php /*?><input type='hidden' name='global_amount' id='global_amount' value='' /><?php */?>
