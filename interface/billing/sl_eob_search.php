...
fe.facility AS Location,b.code AS CPT,b.fee*b.units As Charge,
...
IFNULL(( select sum(w_o) from ar_activity where
 code = b.code and encounter = b.encounter  and (pay_amount>'0.00' or reason_code='wo' ) and payer_type!='0'),0) as w_o,
...
//updated by sonali 10123: Same procedure code
...
function upload_file_to_client_pdf($file_to_send, $aPatFirstName = '', $aPatID = null, $flagCFN = false)
{
 //modified for statement title name
  //Function reads a HTML file and converts to pdf.
	...
	$pdf->ezSetMargins(35,40,36,10);// Sai custom code
  	$pdf->ezStartPageNumbers(320,10,10,'','',1);// Sai custom code
...
// Sai custom code start
	if(stristr($OneLine, 'Billing Statement')  == true || stristr($OneLine, '   STATEMENT   ')  == true)//lines are made bold when 'REMIT TO' or 'Visit Date' is there. 
	 $pdf->ezText('<b>'.$OneLine.'</b>', 12, array('justification' => 'left', 'leading' => 6)); 
	 //lines are made smaller  when condition is matched.
	 else if(stristr($OneLine, 'Dear Patient') == true || stristr($OneLine, 'This provider') == true || stristr($OneLine, 'service patient') == true || stristr($OneLine, 'when they have') == true || stristr($OneLine, 'Please check') == true|| stristr($OneLine, 'information has') == true ){	 
	 	$pdf->ezText('<b>'.$OneLine.'</b>', 6.5, array('justification' => 'left', 'leading' => 6));
	 }
	 else if(stristr($OneLine, 'Phone') == true || stristr($OneLine, 'Account number') == true || stristr($OneLine, 'Patient:') == true || stristr($OneLine, 'Billed to') == true){
	 $pdf->ezText('<b>'.$OneLine.'</b>', 9.5, array('justification' => 'left', 'leading' => 6));
	 }
	 else if(stristr($OneLine, 'Date of Service') == true || stristr($OneLine, 'Provider') == true || stristr($OneLine, 'Location') == true || stristr($OneLine, 'Procedure') == true || stristr($OneLine, 'Charge') == true || stristr($OneLine, 'Primary Insurance Payment') == true){
	 $pdf->ezText('<b>'.$OneLine.'</b>', 9, array('justification' => 'left', 'leading' => 6));
	 }
	 else if(stristr($OneLine, 'Remit This Amount') == true || stristr($OneLine, 'INSURANCE INFORMATION') == true || stristr($OneLine, 'RETURN SERVICE') == true || stristr($OneLine, 'Billing Enquiries') == true){
	 	$pdf->ezText('<b>'.$OneLine.'</b>', 9, array('justification' => 'left', 'leading' => 6));
	 }	
	 else if(stristr($OneLine, '1ST') == true || stristr($OneLine, '2ND') == true || stristr($OneLine, '3RD') == true || stristr($OneLine, 'INSURANCE BALANCE') == true ){
	 	$pdf->ezText('<b>'.$OneLine.'</b>', 9, array('justification' => 'left', 'leading' => 6));
	 }	
	 else if(stristr($OneLine, 'If you have any') == true || stristr($OneLine, 'only through') == true || stristr($OneLine, 'comments') == true){
	 	$pdf->ezText('<b>'.$OneLine.'</b>', 8, array('justification' => 'left', 'leading' => 6));
	 }	 
	 //for displaying table structure
	 else if(stristr($OneLine, '$$$') == true){	
	 	$patient_stat[] = array('Date of Service','Provider','Location','Procedure','Charge','Primary Insurance Payment','Secondary Insurance Payment', 'Adjustment','Balance');	 	
	 	$OneLine = str_replace("$$$", "", $OneLine);
		$newdata = explode("###",$OneLine);
		$pdf->selectFont($GLOBALS['fileroot'] . "/library/fonts/Courier-Bold.afm");		
		for($i=0;$i<count($newdata);$i++){
			$dataarr = explode("==",$newdata[$i]);
			$patient_stat[] = $dataarr;
		}
		
		$pdf->ezTable($patient_stat, null, '',
                array('width'=>'565', 'fontSize'=>'8', 'showLines'=>'2','showHeadings'=>0));	
				$pdf->selectFont($GLOBALS['fileroot'] . "/library/fonts/Courier.afm");				
		unset($patient_stat); // to avoid repeating values in array
	 }
	else
	 $pdf->ezText($OneLine, 9, array('justification' => 'left', 'leading' => 6)); 
	 
	$countline++; $cntPatient_state++;
   }
// Sai custom code end
   ...
   //
    ...
    while ($row = $rows[$rcnt++]) {
       ...
       //
        ...
        fwrite($fhprint, create_statement($stmt,$parameter));// Sai custom code
            $stmt['cid'] = $row['pid'];
            $stmt['pid'] = $row['pid'];
		...
		$stmt['to'][] =  $row['phone_home'] ;// Sai custom code
            $stmt['lines'] = array();
            $stmt['amount'] = '0.00';
            $stmt['ins_paid'] = 0;
            $stmt['today'] = $today;
	...
	/*
      //commented for adding more fields
...
<body leftmargin='0' topmargin='0' marginwidth='0' marginheight='0'>
  ...
  // Identify the payer to support resumable posting sessions.
    ...
    onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc,'<?php echo date("m/d/Y"); ?>')'
    title='<?php xl("Date of payment yyyy-mm-dd","e"); ?>'><!-- Sai custom code -->
       </td>
    ...
    onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc,'<?php echo date("m/d/Y"); ?>')'
    title='<?php xl("Date of bank deposit yyyy-mm-dd","e"); ?>'><!-- Sai custom code -->
       </td>
    ...
    title='<?php xl("Paid amount that you will apply","e"); ?>'><!-- Sai custom code -->
       </td>
       <td align='right'>
...
if ($_POST['form_search'] || $_POST['form_print']) {
 ...
 // Code change by Mahesh. updated code to get proper write off data in posting screen
...
// Sai custom code start
    $query = "SELECT f.id, f.pid, f.encounter, f.date, " .
      ...
      "p.fname, p.mname, p.lname, p.pubpid,p.billing_note, p.genericname2, p.genericval2, " .
      "( SELECT SUM(b.fee*b.units) FROM billing AS b WHERE " .
	  ...
	  "( select sum(a.w_o) from ar_activity as a where " .
      " a.encounter = f.encounter and payer_type!='0') AS w_o " .
...
<table border='0' cellpadding='1' cellspacing='2' width='98%'>
 ...
 <td class="dehead" align="right">
   <?php xl('W/O','e'); ?>&nbsp; <!-- Sai custom code -->
  </td> <!-- Sai custom code -->
  <td class="dehead" align="right"> <!-- Sai custom code -->
    <?php xl('Balance', 'e'); ?>&nbsp;
 </td>
...
while ($row = sqlFetchArray($t_res)) {
      $balance = sprintf("%.2f", $row['charges'] + $row['copays'] - $row['payments'] - $row['adjustments'] - $row['w_o']); // Sai custom code
  ...
  <tr bgcolor='<?php echo $bgcolor ?>'>
   ...
   <td class="detail" align="right"> <!-- Sai custom code -->
   <?php bucks($row['w_o']) ?>&nbsp; <!-- Sai custom code -->
   </td>
   <td class="detail" align="right">
        <?php bucks($balance); ?>&nbsp;
   </td>
