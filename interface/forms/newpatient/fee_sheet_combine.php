...
//require_once("$rootdir/forms/fee_sheet/codes.php");
require_once("../../../custom/code_types.inc.php");
require_once("../../drugs/drugs.inc.php");
require_once("$srcdir/formatting.inc.php");
require_once("$srcdir/ajax/facility_ajax_jav.inc.php");
//require_once("fee_sheet_ajax.php");
  ...
  $billed = FALSE, $code_text = NULL, $justify = NULL, $provider_id = 0)
{
  ...
  if (! $code_text) {
    ...
    $code_text = $result['code_text_short'];
  ...
  if ($billed) {
    if (modifiers_are_used(true)) {
      echo "  <td class='billcell'>$strike1$modifier1$strike2" .
        "<input type='hidden' name='bill[$lino][mod1]' value='$modifier1'></td>\n";
		echo "  <td class='billcell'>$strike1$modifier2$strike2" .
        "<input type='hidden' name='bill[$lino][mod2]' value='$modifier2'></td>\n";
		echo "  <td class='billcell'>$strike1$modifier3$strike2" .
        "<input type='hidden' name='bill[$lino][mod3]' value='$modifier3'></td>\n";
		echo "  <td class='billcell'>$strike1$modifier4$strike2" .
  ...
  /*echo "<script type='text/javascript'>newDiagJS1('$codetype','$code');</script>";	*/
    if (modifiers_are_used(true)) {
      if ($codetype != 'COPAY' && ($code_types[$codetype]['mod1'] || $modifier1)) {
        echo "  <td class='billcell'><input type='text' name='bill[$lino][mod1]' " .
          "value='$modifier1' size='" . $code_types[$codetype]['mod1'] . "'></td>\n";
      }if ($codetype != 'COPAY' && ($code_types[$codetype]['mod2'] || $modifier2)) {
        echo "  <td class='billcell'><input type='text' name='bill[$lino][mod2]' " .
          "value='$modifier2' size='" . $code_types[$codetype]['mod2'] . "'></td>\n";
      }if ($codetype != 'COPAY' && ($code_types[$codetype]['mod3'] || $modifier3)) {
        echo "  <td class='billcell'><input type='text' name='bill[$lino][mod3]' " .
          "value='$modifier3' size='" . $code_types[$codetype]['mod3'] . "'></td>\n";
      }if ($codetype != 'COPAY' && ($code_types[$codetype]['mod4'] || $modifier4)) {
        echo "  <td class='billcell'><input type='text' name='bill[$lino][mod4]' " .
          "value='$modifier4' size='" . $code_types[$codetype]['mod4'] . "'></td>\n";
      } else {
        echo "  <td class='billcell'>&nbsp;</td>\n";
		echo "  <td class='billcell'>&nbsp;</td>\n";
		echo "  <td class='billcell'>&nbsp;</td>\n";
    ...
    if (fees_are_used()) {
      if ($codetype == 'COPAY' || $code_types[$codetype]['fee'] || $fee != 0) {
        ...
        if ($codetype != 'COPAY') {
          ...
          "value='$units' size='2' style='text-align:right' onblur='total_charges_calc();'>";
        } else {
        ...
        echo " onblur='total_charges_calc();'></td>\n";
		$total_charges = $units * $price;
		echo "<td class='billcell' align='right'><input type='text' name='bill[$lino][total_charges]' id='bill[$lino][total_charges]' " .
        ...
        if ($code_types[$codetype]['just'] || $justify) {
          ...
          echo "</td>\n";
          $justinit1 .= "setJustify1(f['bill[$lino][justify]']);\n";
        } else {
  ...
  $fee = NULL, $sale_id = 0, $billed = FALSE)
{
  ...
  if ($billed) {
    ...
    if (fees_are_used()) {
      ...
      echo "  <td class='billcell' align='center'>";
      echo "<input type='text' name='prod[$lino][units]' id='prod[$lino][units]' " .
        "value='$units' size='2' style='text-align:right'>";
      echo "</td>\n";
	   echo "  <td class='billcell' align='right'>" .
        "<input type='text' name='prod[$lino][price]' id='prod[$lino][price]' " .
        "value='$price' size='6'";
      if (acl_check('acct','disc'))
        echo " style='text-align:right'";
      else
...
if ($_POST['bn_save']) {
  ...
  for ($lino = 1; $bill["$lino"]['code_type']; ++$lino) {
    ...
    if ($id) {
      if ($del) {
        deleteBilling($id);
        sqlInsert("insert into log_audit (moduleNumber, billingID, createdDate) values ('fsc2', ".$id.", now())");	
      }
      else {
...
$billresult = getBillingByEncounter($pid, $encounter, "*");
  ...
  $res = sqlStatement("select ct_key,code from codes,code_types where codes.code_type=code_types.ct_id and ct_key='cpt4' ");
	$row = sqlFetchArray($res);
	$CPTKey=$row['ct_key'];
	$CPTCode=$row['code'];
	
	$res = sqlStatement("select ct_key,code from codes,code_types where codes.code_type=code_types.ct_id and ct_key='icd9' ");
	$row = sqlFetchArray($res);
	$ICD9Key=$row['ct_key'];
	$ICD9Code=$row['code'];
...
if(document.getElementById('ajaxCPTdiv').innerHTML==''){
	if(document.getElementById('cpt_table').innerHTML==''){
	lastRow=1;
	}
	else
	{
	lastRow=document.getElementById('cpt_table').rows.length-1;	
	}
	
}
else{
	if(document.getElementById('cpt_table'))
	firstRow=document.getElementById('cpt_table').rows.length-2;	
	allTables= document.getElementById('ajaxCPTdiv').getElementsByTagName("table");	
	var brow=((navigator.appName) + (parseInt(navigator.appVersion)));
	var n=brow.match(/Internet Explorer/g); 
	...
	{ 
		...
		if ($.inArray(selobj, codes_array) != -1) {
		alert("Code already entered!!!");
		document.getElementById('cpt_code').focus();	
		 }
		else{
		codes_array.push(selobj);	
		ajax_code_display(selobj,lastRow);
		document.getElementById('cpt_code').focus();	
		}
	}
}
...
function setJustify(seljust) {
 ...
 function handlecptDiv(obj)
 {

  if(obj.value.length == 0)
{
document.getElementById('ajax_div_cpt').style.visibility = 'hidden';
	}
	else
	{
document.getElementById('ajax_div_cpt').style.visibility = 'visible'; 
    document.getElementById('ajax_div_cpt').innerHTML;
   }
 }
function handleicd9Div(obj)
 {

  if(obj.value.length == 0)
{
document.getElementById('ajax_div_icd9').style.visibility = 'hidden';
	}
	else
	{
document.getElementById('ajax_div_icd9').style.visibility = 'visible'; 
    document.getElementById('ajax_div_icd9').innerHTML;
   }
 }
 
 function validateCPT4(cpt_code,evt){
	...
	{			
	   ...
	   codeselect(cpt_code);		   
	   document.getElementById('cpt_code').focus();	
	   document.getElementById('cpt_code').value='';	
...
<body class="body_top">
<?php /*?><form method="post" action="<?php echo $rootdir; ?>/forms/fee_sheet/fee_sheet_combine.php"
 onsubmit="return validate(this)">
<span class="title"><?php xl('Fee Sheet','e'); ?></span><br><?php */?>
...
?>
<table width="50%" cellspacing='5' border='0'><tr><td colspan="2" class="bold"> Enter Code </td><!--<td colspan="2" class="bold"> ICD9 Code </td>-->
</tr><tr>
  <td>
<?php /*?> <input type="hidden" id="hidden_ajax_close_value" value="<?php echo htmlspecialchars($CPTCode);?>" /><input name='cpt_code'  id='cpt_code' class="text"  style="width:120px"   onKeyDown="PreventIt(event)" onKeyUp="handlecptDiv(this)" onKeyPress="return validateCPT4(this)"  value=""  autocomplete="off"   /><?php */?>

 <input type="hidden" id="hidden_ajax_close_value" value="<?php echo htmlspecialchars($CPTCode);?>" /><input name='cpt_code'  id='cpt_code' class="text"  style="width:120px" onKeyDown="PreventIt(event)" onKeyUp="handlecptDiv(this);return validateCPT4(this.value,event)" autocomplete="off"   />


              <div id="ajax_div_cpt" style="display:none;"></div>	
 </td><td><input type="button" name="add_cpt" id="add_cpt" onClick="codeselect(document.getElementById('cpt_code').value)" value="Add Code" >&nbsp;&nbsp;&nbsp;&nbsp;<i>[Press F2 to add code]</i></td>
  
  <?php /*?><td >
 <input type="hidden" id="hidden_ajax_close_value" value="<?php echo htmlspecialchars($ICD9Code);?>" /><input name='icd9_code'  id='icd9_code' class="text"  style="width:120px"   onKeyDown="PreventIt(event)" onKeyUp="handleicd9Div(this)"  value=""  autocomplete="off"   />
              <div id="ajax_div_icd9" style="display:none;"></div>	
  </td><td><input type="button" name="add_icd9" id="add_icd9" onClick="codeselect(document.getElementById('icd9_code').value)" value="Add ICD9 Code" ></td><?php */?>
  </tr></table> 
<?php /*?><table width='95%'>
<?php
$i = 0;
...
<table width="100%" id="cpt_table" border='0' cellspacing='5' style="background-color:#3399CC">
 <tr>
  <td class='billcell' align='center' width="10%"><b><?php xl('Type','e');?></b></td>
  <td class='billcell' align='center' width="10%"><b><?php xl('Code','e');?></b></td>
<?php if (modifiers_are_used(true)) { ?>
  <td class='billcell' align='center' width="5%"><b><?php xl('Mod1','e');?></b></td>
   <td class='billcell' align='center' width="5%"><b><?php xl('Mod2','e');?></b></td>
    <td class='billcell' align='center' width="5%"><b><?php xl('Mod3','e');?></b></td>
     <td class='billcell' align='center' width="5%"><b><?php xl('Mod4','e');?></b></td>
<?php } ?>
<?php if (fees_are_used()) { ?>
    <td class='billcell' align='center' width="10%"><b><?php xl('Units','e');?></b></td>
    <td class='billcell' align='center' width="10%"><b><?php xl('Units Charge','e');?></b>&nbsp;</td>
   <td class='billcell' align='center' width="10%"><b><?php xl('Total charges','e');?></b></td>
  <td class='billcell' align='center' width="10%"'<?php echo $usbillstyle; ?>><b><?php xl('Diagnosis Pointer','e');?></b></td>
<?php } ?>
  <td class='billcell' align='center' width="10%"><b><?php xl('Provider','e');?></b></td>
  <td class='billcell' align='center' width="10%"<?php echo $usbillstyle; ?>><b><?php xl('Auth','e');?></b></td>
  <td class='billcell' align='center' width="10%"><b><?php xl('Delete','e');?></b></td>
  <?php /*?><td class='billcell' align='center' width="10%"><b><?php xl('Description','e');?></b></td><?php */?>
 </tr>
...
echo "<tr><td colspan='10'><div id='ajaxCPTdiv'>";
/*echoLine11(++$bill_lino, $iter["code_type"], $iter["code"], trim($iter["mod"]),
      $ndc_info, $iter["auth"], $iter["del"], $units,
      $fee, NULL, FALSE, NULL, $iter["justify"], 0 + $iter['provid']);*/
echo"</div><td></tr>";
...
<br />
&nbsp;
<input type="hidden" value="<?php echo $bill_lino; ?>" name="bill_lino" id="bill_lino">
...
<?php if (!$isBilled) { ?>
<?php /*?><input type='submit' name='bn_save' value='<?php xl('Save','e');?>' /><?php */?>
&nbsp;
<?php /*?><input type='submit' name='bn_refresh' value='<?php xl('Refresh','e');?>'><?php */?>
&nbsp;
<?php } ?>

<?php /*?><input type='button' value='<?php xl('Cancel','e');?>'
 onclick="top.restoreSession();location='<?php echo "$rootdir/patient_file/encounter/$returnurl" ?>'" /><?php */?>
