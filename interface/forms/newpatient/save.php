...
$user = sqlQuery("select username from users where username='" . $_SESSION{
"authUser"} . "'");
$uname = current($user);
...
if ($facility_id) {
	...
	//added this line to solve PAYEHR-515 from root.
	$facility = htmlspecialchars_decode($facility);
}
...
'$encID','$enc_status',NOW(),'$modifier_user')";
       ...
       $session_id = idSqlStatement("INSERT INTO ar_session(payer_id,user_id,pay_total,payment_type,description,".
		  ...
		  $billed=1;
						...
						SqlStatement(
							"INSERT INTO ar_activity (pid,encounter,code,modifier,payer_type,post_time,post_user,session_id," .
            ...
            array($_SESSION['authId'],$fee,$form_method,$form_source,$session_id));*/
							sqlStatement(
								"UPDATE ar_activity SET code=?, modifier=?, post_user=?, post_time=now()," .
  ...
  */
			...
			if ($mode == 'new') {
				...
				if ($copay_update == TRUE && $update_session_id != '' && $mod0 != '') {
					sqlStatement(
						"UPDATE ar_activity SET code=?, modified_time=now(),modifier=?" .
	...
	if ($Total_price == 0) {
		...
		foreach ($bill as $key => $iter) {
			...
			if ($iter['id'] && !$iter['code']) {
				$moduleNumber = 'save1';
				deleteBilling($iter['id'], $moduleNumber);
			}
			...
			if ($code_type == 'COPAY') {
				...
				$res_amount = sqlQuery(
					"SELECT pay_amount FROM ar_activity WHERE pid=? AND encounter=? ",
					array($pid, $encounter)
				);
				$memo = "Copay : $" . $fee;
				if ($session_id)
...
<body>
	<script language='JavaScript'>
		<?php if ($GLOBALS['concurrent_layout']) { //Encounter details are stored to javacript as array.
		?>
