...
if ($_POST['bn_save']) {
  ...
  for ($lino = 1; $bill["$lino"]['code_type']; ++$lino) {
    ...
    if ($id) {
      if ($del) {
        deleteBilling($id);
        sqlInsert("insert into log_audit (moduleNumber, billingID, createdDate) values ('fsc1', ".$id.", now())");
      }
      else {
...
function codeselect(selobj) {
 ...
 // f.newcodes.value = 'CPT4|'.concat(selobj,'|');
  //alert(f.newcodes.value);
 // f.submit(); 
 ajax_code_display(selobj);
}
...
<?php /*?><form method="post" action="<?php echo $rootdir; ?>/forms/fee_sheet/fee_sheet_combine.php"
 onsubmit="return validate(this)"><?php */?>
<span class="title"><?php xl('Fee Sheet','e'); ?></span><br>
<input type='hidden' name='newcodes' value=''>

<center>
<table><tr><td class="bold"> Code: &nbsp;
 <input type="hidden" id="hidden_ajax_close_value" value="<?php echo htmlspecialchars($CPTCode);?>" /><input name='cpt_code'  id='cpt_code' class="text"  style="width:120px"   onKeyDown="PreventIt(event)"  value="<?php echo htmlspecialchars($CPTCode);?>"  autocomplete="off"   /><br>	
        <div id="ajax_div_cpt" style="display:none;"></div>	
  </td><td><input type="button" name="add_cpt" id="add_cpt" onClick="codeselect(document.getElementById('cpt_code').value)" value="Add CPT" ></td></tr></table>      
<?php
...
echo "<td><div id='ajaxCPTdiv'>";
echo "<tr>
  <td class='billcell'>CPT4<input type='hidden' name='bill[0][code_type]' value='CPT4'><input type='hidden' name='bill[0][code]' value='99212'><input type='hidden' name='bill[0][billed]' value=''></td>
</tr>";
